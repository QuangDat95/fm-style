...
$data->config();
$data->access();
 $ngaykhoa= gmdate('Y-m-d', time() - 17*3600) ; $thangnamngay=gmdate('ynd', time() -17*3600) ;  // lui lại 1 ngày
...
while($re = $data->fetch_array($result))
	...
	// echo $re['tongtien'] -  $re['vc']. "<br>";
	  $sql = "   SELECT ID from thuchikt where  sochungtu='$sochungtu' ";$tam=getdong($sql);
 //	  echo  $sql . "<br>";
 //	  echo laso($tam['ID']) ;
	 if (laso($tam['ID'])==0)
	 {
  ...
  echo $sql."<br>"; 
      $data->query($sql); 
	  ///thu chi kt
		$sql = "    INSERT INTO thuchikt (IDcha,sochungtu,ngaythuchi,ngaytao,note,sotien ,lydo,nguoinhan,nguoichi,donvi,loaitk,IDtao,IDSua,luachon,lydoN,tinhtrang,idkhoa,ngaykhoa,tkno,tkco,psno,soluong,dongia,psco) VALUES ('97','$sochungtu','$ngaykhoa','$ngaykhoa','Tự động tạo từ phần mềm','$re[tongtien]','Doanh Thu tự động từ phần mềm','','','','$idkho','1','1','1','Tu-dong-tao tu phan mem',4,1,'$ngaykhoa','2517','2734','$re[tongtien]','','','')";
  		echo $sql."<br>"; 
      $data->query($sql); 
