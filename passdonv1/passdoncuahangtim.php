...
left join passdonchitiet b on b.IDphieu=a.ID left join hanghoacuahang d on d.IDSP=b.IDsp".$sql_where." ORDER BY  a.idtao ,a.ngaytao desc,d.IDcuahang   ";*/
	...
	$sql ="
SELECT  a.*,a.ID as idphieu,a.GhiChu as ghichudon,a.SoCT as sochungtu,DATE_FORMAT(a.ngaytao,'%d/%m/%y %h:%i') as ngaytao,DATE_FORMAT(a.ngayhuy,'%d/%m/%y %h:%i') as ngayhuy,a.lydohuy,u.ten,u.manv,c.Name as tenKH,c.tel as sodtkh,c.address as diachikh 
FROM passdon a left join userac u on a.idtao=u.id left join customer c on a.IDNhaCC=c.ID ".$sql_where."    ORDER BY  a.idtao ,a.ngaytao desc limit 100";
   }
   echo "okok<br>";
echo $sql;
...
<?php
 ...
 // xử lý lọc đơn có
	...
	$mangall=array($mangcohangdu1,$mangcohangdu2,$mangidtao);
	/*if($re['IDTao']==$idk){
 ...
 $ghichunhandon='';
  	foreach($re["chitietsp"] as $key =>$value){
				if($value["cuahangnhanhet"]){
					 ...
					 $ghichunhandon='<span>'.$value['GhiChu'].'</span>';
					 ...
					 $ghichunhandon.='<span>'.$value['GhiChu'].'</span><br>';
	...
	if(!$danhanhet || $re['IDTao']==$idk || $tamcheckdonch>0 ){
 	 ?>
		  ...
		  <?php echo $re[ten] ;?>
            ...
            <td style="color:#009933;font-weight:bold" id="tinhtrangnhandon<?php echo $re['ID'];?>"><?php echo $tinhtrangnhandon;?></td>  
				...
				<span><span class="infsp" style="color:<?php echo  $mauchuco; ?>;font-weight:500">Chiếc khấu:</span> <?php echo $value["chietkhau"];?><span class="infsp" style="color:<?php echo  $mauchuco; ?>;font-weight:500"> Giá CK:</span> <?php
					$giachieckhau=$value["dongia"]-(($value["dongia"]/100)*$value["chietkhau"]);
				 echo number_format($giachieckhau*$value["SoLuong"]);?></span></br>
			<?php 
				if($chnhan){
				?>
			...
			?>
               ...
               <span><span class="infsp" style="color:<?php echo  $mauchuco; ?>;font-weight:500">Ghi Chú:</span> <?php echo $re["ghichudon"];?></span></br></td>
				<td ><?php echo formatso($tongtien) ;?></td> 
                <td ><?php echo $ghichunhandon;?></td>    
				...
				<?php
					if($huydonch){
						...
						$clickxn='onclick="cuahangnhandon('.$re['ID'].',1,\''.$chuoichitietnhan.'\',\'\',\''.$dudon.'\',\''.$chuoiidsp.'\',\''.$re['sochungtu'].'\')"';
						$clickhuy='onclick="cuahangnhandon('.$re['ID'].',8,\''.$cuahangnhan.'\',\'\',\''.$dudon.'\',\''.$chuoiidsp.'\',\''.$re['sochungtu'].'\')"';
					}	
				?>
		  ...
		  <?php if($hienxacnhan && ($re['tinhtrang']!=4 &&  $re['tinhtrang']!=3)) {?>
		   ...
		   ?>
		    ...
		    <td >  
             ...
             <button type="button" <?php echo $disbledkhoa;?> id="btnkhoa<?php echo $re['ID'];?>" onclick="cuahangnhandon(<?php echo $re['ID'].",4" ;?>,'','','<?php echo $dudon;?>','<?php echo $chuoiidsp;?>','<?php echo $re['sochungtu'];?>','<?php echo $re["ghichudon"];?>')">Khóa phiếu</button>
			 <?php
			 }
			 ?>
			 </td> 
...
{ 
	global $data ;
		...
		$mangtam=[];
		while($re=$data->fetch_array($query)){
			$mangtam[$re['IDSP']]=array("ID"=>	$re['ID'],"ten"=>	$re['tenpt'],"mahang"=>	$re['mahang'],"dongia"=>$re['DonGia'],"SoLuong"=>	$re['SoLuong'],"chietkhau"=>	$re['chietkhau'],"GhiChu"=>	$re['GhiChu'],"size"=>	$re['size'],"mau"=>	$re['mau'],"IDchnhan"=>	$re['IDchnhan']);
		}	
