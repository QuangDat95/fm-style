 ...
 function chuyen($tam)
  ...
  $tmp = explode('*@!',$data1);
   ...
   $NgayTao = date('Y-m-d H:i:s') ;
	...
	$sql ="
SELECT  a.ID as idphieu,a.IDbill,b.SoCT as sctbill,DATE_FORMAT(a.ngaytao,'%d/%m/%Y %h:%i') as ngaytao,a.IDpassdon,a.mavd,a.tinhtrang,DATE_FORMAT(a.ngaynhandon,'%d/%m/%Y %H:%i') as ngaynhandon,DATE_FORMAT(a.ngaytoikho,'%d/%m/%Y %h:%i') as ngaytoikho,DATE_FORMAT(a.ngaygiaohang1,'%d/%m/%Y %h:%i') as ngaygiaohang1,DATE_FORMAT(a.ngaygiaohang2,'%d/%m/%y %H:%i') as ngaygiaohang2,DATE_FORMAT(a.ngaygiaohang3,'%d/%m/%Y %H:%i') as ngaygiaohang3,DATE_FORMAT(a.ngayhuy,'%d/%m/%Y %H:%i') as ngayhuy,DATE_FORMAT(a.ngayhoanthanh,'%d/%m/%Y %H:%i') as ngayhoanthanh  from vanchuyenpassdon a left join phieunhapxuat b on a.IDbill=b.ID ".$sql_where."  ORDER BY a.ID desc ";
...
//  var_dump($maspve);
 	 ...
 	 ?>
 	 	<tr class="dong_show" id="dong_<?php echo  $re['idphieu'] ; ?>" title="<?php echo addslashes($re['note']) ?>"   onMouseOver="this.className='<?php echo $hl2; ?>'" onMouseOut="this.className='<?php echo $hl; ?>'"  bgcolor="<?php echo $mau; ?>"  style="color:<?php echo $mangtinhtrangmau[$re['tinhtrang']] ; ?>"   >
		...
		<td id="tinhtrang<?=$re['idphieu']?>"><?=$tinhtrangkhoa?></td>
			...
			<td ><button type="button" <?=$disabled?> id="btnkhoa<?=$re['idphieu']?>" onclick="cuahangnhandon(<?php echo $re['idphieu'].",8," ;?>'','','')">Hủy phiếu</button></td>
