  ...
  $tmp = explode('*@!',$data1);
   ...
   $mangtk = taomang ("dinhkhoan","ID","madinhkhoan"); 
	// $manguser = taomang ("userac","ID","MaNV"); 
	  ...
	  $sql_rows = "SELECT g.SoCT as sohd,DATE_FORMAT(g.ngaynhap,'%d/%m/%y %h:%i:%s') as ngayhdbh,g.idkho,g.IDTao as thungan, g.diachiN,b.Name as tencuahang,b.address as diachicuahang,v.donvivc as dvvc,v.mavd as mavd,k.Name tenkh,k.tel as sdtkh,l.Name as lydotra,g.*,v.phitravc,v.phithukh,v.chuyenkhoan 
FROM phieubanhangluu g left join lydotra l on g.tenkho=l.ID left join vanchuyenonline v on v.IDbill=g.ID  left join cuahang b on g.IDkho=b.id  left join userac c on g.IDtao = c.ID 
  left join customer k on k.ID=g.IDNhaCC    
where 1=1 	".$sql_where_chuaload."  and g.SoCT is not null and g.loai=3  group by l.ma,g.diachiN,g.IDTao,g.idchOL ORDER BY g.ID desc ";
	
	   	//$limit=20;
	 	 $start=($curentpage-1)*$limit;
		
		$sql = "SELECT g.SoCT as sohd,DATE_FORMAT(g.ngaynhap,'%d/%m/%y %h:%i:%s') as ngayhdbh,g.IDTao as thungan, g.diachiN,b.Name as tencuahang,b.address as diachicuahang,v.donvivc as dvvc,v.mavd as mavd,k.Name tenkh,k.tel as sdtkh,l.Name as lydotra,g.*,v.phitravc,v.phithukh,v.chuyenkhoan 
FROM phieubanhangluu g  left join lydotra l on g.tenkho=l.ID left join vanchuyenonline v on v.IDbill=g.ID  left join cuahang b on g.IDkho=b.id  left join userac c on g.IDtao = c.ID 
  left join customer k on k.ID=g.IDNhaCC    where 1=1 
	".$sql_where_chuaload."  and g.SoCT is not null and g.loai=3   ORDER BY g.ID desc  limit $start,$limit";
    ...
    <li><button value="-1" onclick="phantrangAjax(this.value,3)">«</button></li>';
		...
		for($i=$min;$i<=$max;$i++){
			...
			if($curentpage==$i){
				...
				$chuoiphantrang.='<li><button value="'.$i.'" onclick="phantrangAjax(this.value,3,15)" style="background-color:'.$mau.'">'.$i.'</button></li>';
		}
	
	
 $chuoiphantrang.='  <li><button value="-2" onclick="phantrangAjax(this.value,1,15)">»</button></li>
  <li><button value="F" onclick="phantrangAjax('.$totalpage.',2,15)">Cuối</button></li>
  
 <li class="chonsodong"><button type="button" value="" onclick="showChondong()">Số dòng</button>
 	<div class="select_sodong"id="select_sodong">
		<div class="select_sodong_item"><button type="button" value="30" onclick="SendLimit(this.value,15)">30 dòng/trang</button></div>
		<div class="select_sodong_item"><button type="button" value="50" onclick="SendLimit(this.value,15)">50 dòng/trang</button></div>
		<div class="select_sodong_item"><button type="button" value="100" onclick="SendLimit(this.value,15)">100 dòng/trang</button></div>
		<div class="select_sodong_item"><button type="button" value="150" onclick="SendLimit(this.value,15)">150 dòng/trang</button></div>
		<div class="select_sodong_item"><button type="button" value="200" onclick="SendLimit(this.value,15)">200 dòng/trang</button></div>
		<div class="select_sodong_item"><input type="number" value="100" id="sodongnhap" nam="sodongnhap" onkeyup="SendLimitKeyup(event,15)" /></div>
...
<div style="" id="showtb"  >
  ...
  <div class="table_left" id="showtb_xuat" style="width:auto;position: -webkit-sticky;
  ...
  <table  border="0" cellpadding="0" cellspacing="0"  class="tbchuan table_bc" id="dopcccc" style="width:100%;border-collapse:collapse">
    <tr align="center" bgcolor="#F8E4CB" class="fixed-top1">
      ...
      <th width="146" valign="middle"  ><strong>Ngày HĐBH</strong></th>
	    <th width="304" valign="middle"  ><strong>Nhân viên Bán</strong></th>
	 
      <th width="186" valign="middle" ><strong>Thu ngân</strong></th> 
      <th width="153" valign="middle" ><strong>Số hóa đơn</strong></th>
     <th width="153" valign="middle" ><strong>Thông tin KH</strong></th>
		 <th width="153" valign="middle" ><strong>SĐT</strong></th>
		 
		  <th width="153" valign="middle" ><strong>Mã SP</strong></th>
		  <th width="153" valign="middle" ><strong>Đơn giá</strong></th>
		    <th width="153" valign="middle" ><strong>Số lượng</strong></th>
			 <th width="153" valign="middle" ><strong>Chiếc khấu</strong></th>
			  <th width="153" valign="middle" ><strong>Thành tiền</strong></th>
				<th width="153" valign="middle" ><strong>đơn vị vận chuyển</strong></th>
				  <th width="153" valign="middle" ><strong>Mã vận đơn</strong></th>
				   <th width="153" valign="middle" ><strong>Đơn nội thành</strong></th>
				    <th width="153" valign="middle" ><strong>Địa chỉ cửa hàng</strong></th>
					 <th width="153" valign="middle" ><strong>Lý do trả hàng</strong></th>
  ...
  while($re = $data->fetch_array($result))
	...
	if($mangnhanviendaydu[$re["diachiN"]] || $mangnhanviendaydu[$re["thungan"]]){
 		 ...
 		 if($mau=="white"){$mau="#EEEEEE";$hl="Normal4";$hl2="Highlight4";}else{$mau="white";$hl="Normal5";$hl2="Highlight5";}
			...
			<td>'.$r.'</td>
				<td>'.$re["ngayhdbh"].'</td>
				<td>'.$mangnhanviendaydu[$re["diachiN"]]['MaNV'].'</td>
				<td>'.$mangnhanviendaydu[$re["thungan"]]['MaNV'].'</td>
				<td>'.$re["sohd"].'</td>
				<td>'.$re["tenkh"].'</td>
				<td>'.$re["sdtkh"].'</td>
				
						<td>'.$re["mahang"].'</td>
					<td>'.number_format($re["DonGia"]).'</td>
					<td>'.$re["SoLuong"].'</td>
					<td>'.$re["chietkhau"].'</td>	
					<td>'.number_format($re["SoLuong"]*($re["DonGia"]*(1-1*$re["chietkhau"]/100))).'</td>
					<td>'.$re["dvvc"].'</td>
					<td>'.$re["mavd"].'</td>
					<td>'.$re["mavd"].'</td>
					<td>'.$re["diachicuahang"].'</td>
					<td>'.$re["lydotra"].'</td></tr>';
			 
			 /* $tongtiendon+=($re["SoLuong"]*($re["DonGia"]*(1-1*$re["chietkhau"]/100)));
			  $soctam=$re["sohd"];
			  $phitranvctam=$re["phitravc"]?$re["phitravc"]:0;
			    $phithukhtam=$re["phithukh"]?$re["phithukh"]:0;
				 $chuyenkhoantam=$re["chuyenkhoan"]?$re["chuyenkhoan"]:0;
			  $giamgiatam=$re["giamgiad"]?$re["giamgiad"]:0;
			  $chuoidong2='';*/
