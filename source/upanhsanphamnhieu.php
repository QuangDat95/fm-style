...
$template->assign("nhacungcap", composx("nhacungcap", "Name", "ID", "  order by Name "));
...
while($r=$data->fetch_array($query)){
	  ...
	  $chuoimau.='<div>
					<div><input type="checkbox" class="jsmau" name="mau[]" id="mau'.$r["ID"].'" style="cursor:pointer" value="'.$r["ID"].'" data-name="'.$r["Name"].'"  data-ma="'.$r["ma"].'"/></div>
												<label for="mau'.$r["ID"].'" style="margin-left:0.5em;cursor:pointer">'.$r["ma"].'-'.$r["Name"].'</label>
											</div>';
}
...
while($r=$data->fetch_array($query)){
	  ...
	  $chuoisize.='<div>
												...
												<label for="size'.$r["ID"].'" style="margin-left:0.5em;cursor:pointer">'.$r["ma"].'-'.$r["Name"].'</label>
											</div>';
}
