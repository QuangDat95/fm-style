  ...
  $sp = $tmp[1] ;
   ...
   $checksp = $tmp[3] ;
	  $ch = $tmp[4] ;
		...
		if($checksp){
				//check team
			if(!checkTeam(trim($ch))){
				if(!checkCuahang(trim($ch))){
					echo -2;
						return;
				}
		...
		if($checksp){
		$sql="update datapancake set T6='".$sp."',T8='".$sl."',T2='".$ch."' where ID=".$ids;
	
	}
	else{
		$sql="update datapancake set T6='".$sp."',T8='".$sl."' where ID=".$ids;
	}
	if($data->query($sql)){
...
if(isset($_POST['DATA'])){
 $_SESSION['sodongloi']=0;
 	...
 	$data1 = $_POST['DATA']; 
 	 $tmp = explode('*@!',$data1);
	 $checkcol=true;
	 include($root_path."shoppetinsertexel.php");
	 if(!$checkcol){
		return;
	}
}
...
<table id="tbex" width="100%" border="0" cellpadding="0" cellspacing="0" style="background:#FFF;" class="tbchuan">		
 		<tr bgcolor="#F8E4CB">
		  <td align="center"  height="23" width="35"><b>STT</b></td>
		   <td align="center"  height="23" width="43"><b>Team</b></td>
...
$macheck=$r['T1'];
		if($r['T3']){
			...
			//check team
			if(!checkTeam(trim($r["T2"]))){
				if(!checkCuahang(trim($r["T2"]))){
		...
		if($checkvd && !$check){
		?>
		<tr id="<?=$r['ID']?>" style="cursor:pointer;color:<?php echo $mauchu ; ?> " onclick="showPoupSua('<?=$r['ID']?>','<?=$r['T4']."---".$r['T2']?>','<?=$r["T54"]?>','<?=$texterror?>','<?=$r['T6']?>','<?=$r["T8"]?>','<?=$mangvc["sotien"]?>',1,3)">
		...
		<?php		
			...
			?>
				...
				<tr id="<?=$r['ID']?>" style="cursor:pointer;color:<?php echo $mauchu ; ?> " onclick="showPoupSua('<?=$r['ID']?>','<?=$r['T4']."---".$r['T2']?>','<?=$r["T54"]?>','<?=$texterror?>','<?=$r['T6']?>','<?=$r["T8"]?>','<?=["T5"]?>',1,3)">
			<?php
		...
		?>
          ...
          <td  align="left"><?php echo $r["T2"];?></td>
			...
			<td  align="left"><?php echo $r["T6"];?></td>
 			...
 			<td  align="left"><?php echo $r["T8"];?></td>
...
//check sp
		...
		if(!$check){
		?>
		<tr id="<?=$r['ID']?>" style="cursor:pointer;color:<?php echo $mauchu ; ?> " onclick="showPoupSua('<?=$r['ID']?>','<?=$r['T4']."---".$r['T2']?>','<?=$r["T54"]?>','<?=$texterror?>','<?=$r['T6']?>','<?=$r["T8"]?>','<?=$mangvc["sotien"]?>',0,3)">
...
<div style="padding-bottom:10px;padding-left:20px"><input type="button" id="dulieue" name="dulieue"  onclick="xuatbaoloi('Có lỗi trong file tải lên chú ý chổ màu đỏ!')" value="Lấy dữ liệu Excel"/> </div>  
 ...
 {
 ?>
 <div style="padding-bottom:10px;padding-left:20px"><input type="button" id="dulieue" name="dulieue"  onclick="laydulieuexeltmdt()" value="Lấy dữ liệu Excel"/> </div>  
  <?php
 }
