...
$data->access();
 ...
 FROM thuchikt a left join dinhkhoanthuchi d on a.IDcha=d.ID left join cuahang b on a.loaitk=b.id left join tinh c on c.ID=b.Fax where 1=1 ".$sql_where." and b.IDNhomcc <> 8 and (d.ma='KTT' or d.ma='KTTI') group by a.ngaythuchi,a.loaitk order by  a.ngaythuchi desc";
 ...
 FROM thuchikt a left join dinhkhoanthuchi d on a.IDcha=d.ID left join cuahang b on a.loaitk=b.id left join tinh c on c.ID=b.Fax where 1=1  ".$sql_where." and b.IDNhomcc <> 8 and (d.ma='KTT' or d.ma='KTTI') group by a.ngaythuchi,a.loaitk order by  a.ngaythuchi desc limit $start,$limit";
//var_dump($sql);
 ...
 <li>
 <input type="button" onclick="xuatexel(8)" name="search3" style="height:30px;margin-left:5px;" id="search3" value="Excel KTT/T" />
 </li>
  	</ul> </div>
	...
	foreach($mangtam as $key => $value){
	 if($mau=="white"){$mau="#EEEEEE";$hl="Normal4";$hl2="Highlight4";}else{$mau="white";$hl="Normal5";$hl2="Highlight5";}
		...
		foreach($mangchKH as $k => $v){
			...
			if($dongcuoi3<count($mangch)){
				if($mangtong[$v]){
			 		$chuoitong4.='<td width="37" valign="middle"  ><strong>'.$mangtong[$v].'</strong></td>';
						$dongcuoi3++;
				}else{
