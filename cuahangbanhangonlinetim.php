...
$data->access();
 ...
 // if ($ten != "admin" || 1==1) $quyenhuyphieu = " and dakhoa != '2' " ;
  ...
  $nc = laso($tmp[8]) ;	
		...
		if ($idk==1 || $idk== 5562 || $idk== 7576) $sql_where= " where (a.Loai ='1' or a.Loai ='3') $quyenhuyphieu " ;*/
		 
		if($sophieu!="")
		{
				if($nc==0) { $sql_where.=" and a.SoCT  = '".trim($sophieu)."'"; }
				elseif($nc==1) { $sql_where.=" and a.SoCT  like '".trim($sophieu)."%'"; }
				elseif($nc==2) { $sql_where.=" and a.SoCT  like '%".trim($sophieu)."'"; }
				elseif($nc==3) { $sql_where.=" and a.SoCT  like '%".trim($sophieu)."%'"; }
		}
		
 		if($dakhoa=="0" || $dakhoa=="1" ) $sql_where.=" and a.dakhoa  = $dakhoa " ;  
	...
	//	else	$sql_where.=" and a.idtao=1  and a.ngaytao>='2021-11-19'  and a.SoCT like '%td%'  ";
		...
		if($denngay!="")	
		{
 		...
 		$sql = "SELECT a.*,c.Name,DATE_FORMAT(a.NgayTao,'%d/%m/%Y %H:%i') as ngay,sum(b.DonGia*(1-1*b.chietkhau/100)*b.SoLuong) as sotien  FROM passdon a left join passdonchitiet b  on b.IDphieu =a.ID left join  cuahang c on a.IDKho =c.ID ".$sql_where." group by a.id ORDER BY a.NgayTao desc   ";
		 $pagesize = 40; 
		if($tientu>0 && ($idk==1||$ql[5]) )	
		{  $pagesize = 200; 
		   $sql = "select * from (SELECT a.*,c.Name,DATE_FORMAT(a.NgayTao,'%d/%m/%Y %H:%i') as ngay,sum(b.DonGia*(1-1*b.chietkhau/100)*b.SoLuong) as sotien  FROM passdon a left join $tinhtrang b  on b.IDphieu =a.ID left join  cuahang c on a.IDKho =c.ID ".$sql_where." group by a.id ORDER BY a.NgayTao desc ) g where sotien>=$tientu   ";
		}
...
<div style="width:850px;overflow:scroll;height:400px" align="center">
 <table width="820px" border="0" cellpadding="0" cellspacing="0" class="tbchuan">		
 		<tr bgcolor="#F8E4CB">
          ...
          <td width="254" align="center" ><strong>Tư vấn</strong></td>  
...
<?php
$tongtien =0 ;
$mangtuvan =taomang("userac","ID","ten");
...
?> 
		  ...
		  <td     align="left"><?php echo $re['SoCT']  ;?></td>	
				...
				<td ><?php echo $re['tenlydo'] ;?></td>
				 <td><?php echo $mangtuvan[$re['diachiN']] ;?></td>
...
?>	
 ...
 <div style="padding:5px;" >
  Tìm thấy  <?php echo  $num ;  ?>  phiếu !  &nbsp; tổng tiền: <?php echo  formatso($tongtien) ; ?>    <?php if ($num > $pagesize ) echo "Trang : ". $pt ; ?>  
  </div>
