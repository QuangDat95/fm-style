...
$data->access();
  ...
  $tmp = explode('*@!',$data1);
 	...
 	$sql = "SELECT a.ID as IDp, a.soct,a.lydo,a.sotien,a.tinhtrang,a.hinhanh,a.ID,a.idcuahang,c.Ten as nguoitao,a.chucvu,a.manv , b.name as tencuahang ,DATE_FORMAT(a.ngaytao,'%d/%m/%Y') as ngaytao,DATE_FORMAT(a.ngaygiamdoc,'%d/%m/%Y %H:%i') as ngayduyet,a.ghichu,a.lydothumua,a.lydoketoan,a.lydogiamdoc FROM duyetdexuat a left join cuahang b on a.idcuahang=b.id  left join userac c on a.IDTV = c.ID    where a.ID=$idphieu";
// echo $sql;
  ...
  if($re)
	{   
 	    	 $mangtangca[$re['MaNV']]=1;
				...
				elseif ($ketoan==4 )  $tinhtrangduyet="Chờ giám đốc duyệt" ; 
	...
	<div id="showform">
			<div class="form" style="width:100%">
				...
				<div class="block_i">
					...
					<div><label>Số tiền</label><strong>:</strong> <?=formatso($re['sotien'])?></div>
					
				</div>
				<div class="block_i">
					<div><label>Tình trạng</label><strong>:</strong> <span class="tinhtrangform"> <?=$tinhtrangduyet?></span></div>
					
					<div><label>Lý do không duyệt</label><strong>:</strong><span class="break_w">  <?php echo $re['lydothumua']?$re['lydothumua']:"";
    ...
    justify-content: space-around;" class="btn_w">
					 ...
					 <?php } ?> 	 
					  <?php  if($ql[5]) { ?>
					<div class="block_i block_d">
					<div><label>Lãnh đạo duyệt:</label><div style="    display: flex;
    justify-content: space-around;" class="btn_w">
				...
				if($re["hinhanh"]){
								...
								if($value!=""){
									...
									<img style="width:100%" src="images/duyetmuasam/'.$value.'" /></div>';
									}
								}
							 }
					echo $chuoihinh;
