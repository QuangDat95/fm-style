...
$data->access();
  ...
  $tmp = explode('*@!',$data1);
   ...
   $mangtk = taomang ("dinhkhoan","ID","madinhkhoan"); 
	  ...
	  $sql_rows = "SELECT g.SoCT as sohd,g.TiGia as giamgiad,DATE_FORMAT(g.ngaytao,'%d/%m/%y %h:%i:%s') as ngayhdbh,b.Name as tencuahang,a.donvivc as dvvc,a.mavandon as mavd,a.manv,a.note as diengiai,k.Name tenkh,k.tel as sdtkh,i.*
 FROM thuchikt a left join phieunhapxuat g on a.hdbh=g.SoCT  left join xuatbanhang i on g.ID=i.IDPhieu left join cuahang b on a.loaitk=b.id  left join userac c on a.IDtao = c.ID 
  left join dinhkhoanthuchi d on d.ID = a.IDcha left join nhacungcap e on a.NCC=e.ID left join phieuxuat  f on a.phieuxuat=f.ID  left join customer k on k.ID=g.IDNhaCC    
	".$sql_where." and (d.ma='CNDHOLPD' or d.ma='HCNDHOLPD')  and g.SoCT is not null  ORDER BY g.ID desc ";
		...
		$sql = "SELECT g.SoCT as sohd,g.TiGia as giamgiad,DATE_FORMAT(g.ngaytao,'%d/%m/%y %h:%i:%s') as ngayhdbh,b.Name as tencuahang,a.donvivc as dvvc,a.mavandon as mavd,a.manv,a.note as diengiai,k.Name tenkh,k.tel as sdtkh,i.*
 FROM thuchikt a left join phieunhapxuat g on a.hdbh=g.SoCT  left join xuatbanhang i on g.ID=i.IDPhieu left join cuahang b on a.loaitk=b.id  left join userac c on a.IDtao = c.ID 
  left join dinhkhoanthuchi d on d.ID = a.IDcha left join nhacungcap e on a.NCC=e.ID left join phieuxuat  f on a.phieuxuat=f.ID  left join customer k on k.ID=g.IDNhaCC    
	".$sql_where."  and (d.ma='CNDHOLPD' or d.ma='HCNDHOLPD')  and g.SoCT is not null  ORDER BY g.ID desc  limit $start,$limit";
//echo "<br>".$sql ;
//======================================================
  ...
  <table  border="0" cellpadding="0" cellspacing="0"  class="tbchuan table_bc" id="dopcccc" style="width:100%;border-collapse:collapse">
   <tr align="center" bgcolor="#F8E4CB" class="fixed-top1">
      ...
      <th width="146" valign="middle"  ><strong>Ngày HĐBH</strong></th>
	   ...
	   <th width="304" valign="middle"  ><strong>Mã NV</strong></th>
