				...
				if($_SESSION["LoginID"]!=7576 && $_SESSION["LoginID"]!=4647 && !$ql[5]){
						if($ngaytaiqh>$ngayquahanchuan)
					   {
						...
						if($strcrr<=strtotime($loadden) && $strcrr>=strtotime($loadtu)){
							...
							$currdate=date("d",$currdate);
							if(in_array($idkho,$cuahangchophep)){
						
										if($currdate!=$ngayquahanchophep){
											//var_dump($ngayquahanchophep);
											$quahan=true;
				  ...
				  if(!kiemtratontaidulieu(date("Y-m-d",strtotime($r[0])),$r[11],$r[4],$mangch[strtoupper($r[1])],$r[13],$r[9],$r[14],$r[15],$r[16],$r[17],$r[22],$r[5])){
...
*/												}else{
					...
					if($checkloi){
				 	...
				 	$sp++;
						$chuoiinsert.="('$IDCha','$sochungtu','$ngaythuchi','$ngaytao','$note','$dongia','$ngaytao','$lydo','$nguoinhan','$nguoichi','$IDcuahang','$_SESSION[LoginID]','$_SESSION[LoginID]','$luachon','$LydoN','$idnganhang','$loaitaikhoan','$tinhtrang','$IDkhoa','$tkno','$tkco','$psno','$psco','$donvitinh','$soluong','$dongia','$hdbh','$sotknh','$mavandon','$NCC','$manv','$phieuxuat','$sophieupm','$chungtu','$tentknh','$donvivc'),";
					}
...
/*
	...
	$sql = "insert into thuchikt  (IDCha,sochungtu,ngaythuchi,ngaytao,note,sotien,ngaysua,lydo,nguoinhan,nguoichi,loaitk,IDtao,IDsua,luachon,LydoN,idnganhang,loaitaikhoan,tinhtrang,IDkhoa,tkno,tkco,psno,psco,donvi,soluong,dongia,hdbh,sotknh,mavandon,NCC,manv,phieuxuat,sophieupm,chungtu,tentknh,donvivc) values ".$chuoi;
