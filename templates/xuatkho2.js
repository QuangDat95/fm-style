			...
			if (Object.keys(prodarray).length >= SLsanpham) {
				...
				for (t in mang) {
					if (mangsp[mang[t][0]][0] == macu) {
						mang[t][1] = parseInt(mang[t][1]) + parseInt(mang[t][1]);
					} else {
						//mang[t][1] = mang[t][1]; 
			...
			if (apdungcho == "SL") {
				...
				for (x in prodarray) {
					...
					for (i = 1; i <= prodarray[x][2]; i++) {
						...
						if ((prodarray[x][4] * 1) < chietkhau && so >= giatienapdung) {
							a.push([x, so]);
						}
					}
				}
...
function luuphieuxuat() {
	...
	if (thongbao(cf) == true) {
		var idluu, ngayxuatkho, sochungtu, xuatkho, tigia, lydo, khachhang, ghichu, tenkhachhang, tenlydo, idgoi, khachdua, qua, diem, ido, idch, idgioithieu, idchat, lydotra, nvonline;
		...
		if (kiemtraphieu() == true) {
			...
			poststr += "*@!" + encodeURIComponent(idgioithieu) + "*@!" + encodeURIComponent(idchat) + "*@!" + encodeURIComponent(lydotra) + "*@!" + encodeURIComponent(nvonline);
