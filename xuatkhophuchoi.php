...
$idk = laso($_SESSION["LoginID"]) ; if ( $idk != 1) return ;
   ...
   $ngaychan= gmdate('Y-m-d', time() + 7*3600 -96*3600) ;
    ...
    if ( md5($_SESSION["anhtuan"])=="0993b0db8562b2e6162d164afdbd5c35" ||$_SESSION["admin"]==1) $ngaychan= gmdate('Y-m-d', time() + 7*3600 -92248*3600) ; // v l c t   

      $mtam = taomang("xuatbanhang","IDSP","SoLuong"," where IDPhieu = '0$id'  ") ;
	   $mtamx = serialize($mtam);
      $tam = getdong(" select dakhoa, SoCT,IDNhaCC,IDKho,SoCT from phieunhapxuat where ID = $id and NgayTao >= '$ngaychan'   limit 1") ;
...
from phieunhapxuat a left join  xuatbhchuakhoa b on a.ID=b.IDPhieu where 1=1 and a.SoCT='$tam[SoCT]' group by a.SoCT    " ;
   	  ...
   	  $sql = " update phieunhapxuat set dakhoa = '0'    where ID = '0$id'  " ;
	  $data->query($sql);
     
	   $sql = " insert into xuly   set chucnang = 'phục hồi phiếubán hàng đã khóa : $sophieu',loai=1,noidungcu='$mtamx', lydo='$lydo',ngayxuly='$ngayxuly' " ;
 			  $data->query($sql);

	$data->closedata() ;
	
	echo "*#tb#*Đã phục hồi thành phiếu chưa khóa xong !*#tb#*";
  	return ;
