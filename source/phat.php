...
{ 	
		...
		if  ($_GET["id"] == "-1")
		{
			  $sql ="select ID from  kh_phat where sotien ='$sotien' and idnhanvien='$idnhanvien' and ngay  ='$ngay' and Name ='$Name' limit 1 ";
			  if (getdong($sql)=='')
				{	  
				  $sql ="insert into kh_phat SET ngaytao='$ngaytao',Name ='$Name', ngay  ='$ngay' ,sotien ='$sotien',idnhanvien='$idnhanvien' ";
				   // echo  $sql;
		...
		{
		  $sql = "UPDATE  kh_phat  SET  idnhanvien ='$idnhanvien', Name ='$Name', ngay  ='$ngay',sotien ='$sotien' where ID='0$ID'" ;	
		  $update = $data->query($sql);		
		}  
	//	echo $sql ; 
...
{ 
      $ngaychan= gmdate('Y-n-d H:i:s', time() + 7*3600-30*24*3600) ;  
		$IDD = $_GET["Del"] ;
		$sql = "delete from  kh_phat  where ID = '0".$IDD."' and ngaytao >'$ngaychan'" ;
...

{
	...
	if ($_REQUEST["search"] != "" || $them  || $xoa || $_REQUEST["id"]== "" )
	{
		...
		$sql = "SELECT a.ID FROM   kh_phat a left join userac  b on a.idnhanvien =b.ID ";
	...
	// phan trang===================================================================
 	 ...
 	 $sql = "SELECT DATE_FORMAT(a.ngay, '%d/%m/%Y') as ngayung, a.*,b.ten,b.manv FROM kh_phat a left join userac  b on a.idnhanvien =b.ID  ".$sql_where." ORDER BY a.ngay desc ,b.ten desc  ";
//  echo $sql ;
//=========================================================
  $result = $data->query($sql);
    $SOST = 0 ; $tongung =0 ;
		...
		{  
			...
			$template->assign("ngay", $result_news["ngayung"]);  
	...
	{ 
		...
		else		
		{
			...
			$sql ="SELECT DATE_FORMAT(ngay, '%d/%m/%Y') as ngays, a.*,b.ten,b.manv FROM kh_phat a left join userac  b on a.idnhanvien =b.ID   WHERE a.ID='".$_REQUEST["id"]."'";
