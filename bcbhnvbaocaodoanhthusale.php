...
if ($nhom > 0) {
 ...
 if ($tinhtrang == 1) { 	$sql_where .= " and (v.dongthoigiantrangthaidon =1   ) ";}
 elseif ($tinhtrang == 2) { 	$sql_where .= " and (v.mavd='' or v.mavd is null ) and a.loai<>3";}
 elseif ($tinhtrang == 3) { 	$sql_where .= " and (v.dongthoigiantrangthaidon is null or v.dongthoigiantrangthaidon =''  ) and a.loai<>3";}
 elseif ($tinhtrang == 4) { 	$sql_where .= " and  v.loai=8 and a.loai<>3  and  v.dongthoigiantrangthaidon='' ";}
 elseif ($tinhtrang != "" && $tinhtrang != "0") {	$sql_where .= " and v.dongthoigiantrangthaidon = $tinhtrang ";} 
 elseif ($tinhtrang == "0") {	$sql_where .= " and v.dongthoigiantrangthaidon <>1 and v.dongthoigiantrangthaidon <> 8 and  a.loai<>3    ";}
...
{ 
	...
	{ $r ++ ;
	   ...
	   if($dauphay=='') $dauphay="," ;
	   if($r==100) { $r=0;  $dauphay='';  echo $sqlu ."<br>" ; $k++;
	        $data->query($sqlu);	 
			$sqlu=" insert	into vanchuyenonline (idbill,sobill,mavd,phithukh,donvivc,tongtien,dongthoigiantrangthaidon,ngayhoanthanh,loai) values " ;
	    }
	 if($k==10)  return ; 
	}	 
	 if($r!=0)  {  echo $sqlu ."<br>" ;  $data->query($sqlu); }
	 return ;
...
if ($num > 15000) {
	...
	while ($re = $data->fetch_array($result)) {
		...
		if ($re['hoanthanh'] != 0 && $re['tinhtrang'] == 1)  { $hoanthanh++;   $tienhoanthanh +=$re['hoanthanh'] ; }
		if ($re['hoanthanh'] != 0 && $re['tinhtrang'] == 8)   { $donhoan++;  $tiendonhoan +=$re['hoanthanh'] ; }
...
<div style="display:auto;overflow:scroll;min-width:960px;max-width:1450px;height:415px">
 ...
 <?php
			...
			while ($re = $data->fetch_array($result)) {
                ...
                if($re['idchOL']==$re['idgioithieu'] && $re['idgioithieu']>0) { $re['hoanthanh']=1 ; $re['tinhtrang'] = 1 ; $mauchu="#666600" ; }
 				if ($re['SoCT'] == $soct) $re['tigia'] = 0;
				$soct = $re['SoCT'];
				$ten = $re['Name'];
				$ma = $re['codepro'];
