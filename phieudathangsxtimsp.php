...
$data->access();
  ...
  $tmp = explode('*@!',$data1);
	...
	//	$sql = "SELECT * FROM products  ".$sql_where." ORDER BY NgayTao desc  ";
		$sql = " SELECT a.ID,a.Name,a.NameN,a.size,a.mau , b.SoLuong, a.DV, a.NameEN,a.codepro,a.code,a.price ,a.giabinhquan  ";
  ...
  <table width="98%" border="0" cellpadding="0" cellspacing="0" class="tbchuan">		
 		<tr bgcolor="#F8E4CB">
		  ...
		  <td width="690" align="center" ><strong>Tên Hàng Hóa </strong></td>  
		   ...
		   <td width="182" align="center" ><strong>Size</strong> </td> 
		    <td width="182" align="center" ><strong>Màu</strong></td> 
 		    <td width="106" align="center" ><strong> Giá nhập </strong></td>
...
$hl2 = "Highlight5";
 ...
 $ma = addslashes($re['codepro']) ;
  ...
  if (laso($re['giam']) > laso($re['giamgia']) ) $giamgia = $re['giam'] ; else $giamgia = $re['giamgia'] ;
   $giachan = $re['giachan'] ;
 $gia = formatso($re['giabinhquan']) ;
  
 $dvt = $re['DV'] ;
  if ($gia =='0.00') $gia = "";
 
	 ?>
 	 	<tr title="<?php echo $re['note'] ?>"   style="cursor:pointer" onclick="setsanpham('<?php echo $re['ID'] ;?>','<?php echo $ten ;?>','<?php echo $ma ;?>','<?php echo $gia ;?>','<?php echo $dvt ;?>','<?php echo $giamgia ;?>','<?php echo $giachan ;?>','<?php echo $re['SoLuong'] ;?>','<?php echo $re['NameN'] ;?>','<?php echo $giadagiam ;?>','<?php echo $re['ghichu'] ;?>')" onMouseOver="this.className='<?php echo $hl2; ?>'" onMouseOut="this.className='<?php echo $hl; ?>'"  bgcolor="<?php echo $mau; ?>" >
