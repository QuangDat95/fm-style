...
if(isset($_POST["TAOVANDON"])){
	...
	echo "<pre>";
	$mavd="";
	$mavdres=$ghtk->CreatBillCode($mangvandon);
	if($mavdres["success"]){
...
if(isset($_POST["huydon"])){
	...
	if(!isset($_FILES["hinh"]) || isset($_FILES["hinh"])==''){
					...
					echo "###8###Vui lòng chọn hình ảnh!###-1###";
					return;
	}
...
/*	$tench=getten("cuahang",$idcuahang,"Name");
	...
	//echo $sql."<br>";
	 ...
	 $mavd=$dong["mavd"];
	// echo $mavd."<br>";
		//$huyvandon=$ghtk->CancleBillCode($mavd);
		$huyvandon["message"]="";
			...
			// echo $update;
			 if($update){
			 	foreach($xuatbanhang as $key => $value){
					...
					$sql="update passdon  set tinhtrang=3,ngayhuy='$ngay',lydohuy='$lydo',images='$chuoihinh' where ID='$idpassdon'";
				
		
				$update= $data->query($sql);
				if($update)
				{
					 echo  "###8###Đã hủy phiếu! ".$huyvandon["message"]."###$id###" ;
				 }
			 }
			 ...
			 echo  "###8###Có lỗi xảy ra vui lòng thử lại!\n".$huyvandon["message"]."###-1###";
		}	
		 }
		 else{
		 	 echo  "###8###Có lỗi xảy ra vui lòng thử lại!\n".$huyvandon["message"]."###-1###";
...
select IDkho,IDNhaCC,IDNhap,'$ngay','$soct',LyDo,SoNgayNo,IDTKNo,IDTKCo,TiGia,VAT,GhiChu,'$ngay',IDTao,NguoiGiao,3,ten,diachi,tenkho,tenlydo,tenN,diachiN,dakhoa,nguoitao,nguoisua,ngaykhoa,dahuy,IDkhoa,tientra,idchOL,idgioithieu from phieunhapxuat where ID='$id'";
//echo $sql;
$update= $data->query($sql);
...
$idbillmoi=$dong['ID'];
		$sql="select * from xuatbanhang where IDPhieu='$id'";
				...
				while($re=$data->fetch_array($query)){
						$insert.="('".$idbillmoi."','".$re['IDSP']."','".$re['mahang']."','-".$re['SoLuong']."','".$re['DonGia']."','".$re['LoaiTien']."','".$re['tenpt']."','".$re['thue']."','".$re['BaoHanh']."','".$re['GhiChu']."','".$re['Loai']."','".$re['giavon']."','".$ngay."','".$re['IDtao']."','".$re['IDnhom']."','".$re['IDNV']."','".$re['chietkhau']."','".$re['mota']."'),";
				}
