...
$data->access();
     ...
     $pass =  chonghack($pass)  ;	
     if(laso($idphong)>0)  $phong = " b.idphong=$idphong and " ; 
	 
 	 $sql = "select * from (SELECT  a.idnhanvien ,a.ngaytao, b.idphong from nhanviendilam a left join userac b on a.idnhanvien=b.id  where $phong a.ngaytao>='$ngaytao' order by a.ngaytao desc ) a  group by a.idnhanvien  ";  
	 $result = $data->query($sql);
     while($re = $data->fetch_array($result))  { $mangqt[$re['idnhanvien']]= $re['ngaytao'] ; }
