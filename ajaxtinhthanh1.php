...
if(isset($_POST["TINH"])){
	...
	$idtinh=$tmp[0];
	$sql="select * from quan1 where idtinh=".$idtinh;
...
if(isset($_POST["THANH"])){   
	...
	$idtinh=$tmp[0];
	$idthanhpho=$tmp[1];
	$sql="select * from phuong1 where idtinh=".$idtinh." and idquan=".$idthanhpho;
...
if(isset($_POST["PHUONG"])){   
   ...
   $idthanhpho=$tmp[1];
   $idquan=$tmp[2];
   $sql="select * from duong1 where idtinh=".$idtinh." and idquan=".$idthanhpho."idquan=".$idquan;
   echo $sql;
   $query_districts = $data->query($sql);
   $arrtmp=[];
