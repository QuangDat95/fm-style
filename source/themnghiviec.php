...
{ 		$tongtien=1;
     ...
     WHERE TABLE_SCHEMA = '$db[name]' AND TABLE_NAME = 'duyetnghiviec';" ;  
	 $kq = getdong($sql) ;		
    	$sp  = laso($kq['sp']);	
		...
		if($_GET["id"] == "-1")
		{
 		  ...
 		  $sql ="insert into duyetnghiviec set soct='$sochungtu',lydo='$lydo',manv='$manv',ngaytao ='$ngaytao',IDtao=$IDnhanvien,tinhtrang=11,tinnhan=11,IDNV = '$IDnhanvien',hinhanh='$hinhanh',IDcuahang='$IDcuahang',ngaynghidexuat='$ngaynghidexuat',ghichu='$ghichu'";
...
";
			...
			/*	$result=sendme($id,$noidung);
				...
				if($result){
					...
					if($result['status']==200){
						$sql="UPDATE  duyetnghiviec  set tinnhan=21 where manv='$manv' and ngaytao ='$ngaytao' and IDtao=$IDnhanvien and lydo='$lydo'"  ;
						$update = $data->query($sql);
					}
				}	*/
			}
		...
		{	
		  ...
		  $sql = "UPDATE  duyetnghiviec  set lydo='$lydo',manv='$manv',ngaytao ='$ngaytao',IDtao=$IDnhanvien,tinhtrang=11,IDNV = '$IDnhanvien',hinhanh='$hinhanh',IDcuahang='$IDcuahang',ngaynghidexuat='$ngaynghidexuat',ghichu='$ghichu'  where ID=".$_REQUEST["id"]."  and (IDtao=$IDnhanvien or idtao=7218) and  tinhtrang<44  "  ;	
...
if ($del>0  )
{
	...
	{ 
			...
			$sql = "delete from  duyetnghiviec where  ID = '0".$IDD."'  $dieukiem " ;
...

{
	...
	if  ($_REQUEST["id"] == ""  || $them  || $xoa || $_POST["search"] != "" )
	{
		...
		$sql = "SELECT a.id FROM duyetnghiviec a ";
 		$sql.=$sql_where;
		//echo $sql ;
