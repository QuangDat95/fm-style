 ...
 function chuyen($tam)
  ...
  $tmp = explode('*@!',$data1);
   ...
   else
   {
	...
	$sql ="
SELECT  a.*,a.ID as idphieu,a.GhiChu as ghichudon,a.ngaytao as ngaytaogoc,a.SoCT as sochungtu,DATE_FORMAT(a.ngaytao,'%d/%m/%y %h:%i') as ngaytao,DATE_FORMAT(a.ngayhuy,'%d/%m/%y %h:%i') as ngayhuy,a.lydohuy,u.ten,u.manv,c.Name as tenKH,c.tel as sodtkh,c.address as diachikh 
...
$dudon='';
	...
	if($key==0 && $sophut<=$thoigiannhandon){
	 		$dudon='all';
	 }
		  ...
		  <td ><?php echo $re['ngaytao'] ;?><br />
           ...
           <td ><?php echo $re[ten] ;?></td>			
            ...
            <td style="color:#009933;font-weight:bold" id="tinhtrangnhandon<?php echo $re['ID'];?>"><?php echo $tinhtrangnhandon;?></td>  
				...
				/*	$maubgco='';
					...
					if($sophut>=$thoigiannhandon){
						...
						$clickxn='onclick="nhandonle('.$re['ID'].',1,\''.$value["ID"].'\',event,\'\',\''.$value["ID"].'\')"';	
						}
			...
			<?php
				...
				?>
				<input type="checkbox" style="cursor:pointer" <?php echo $clickxn;?> id="check_<?php echo $value["ID"]; ?>"  <?php echo $disabled; echo $checked;?>/>
			<?php
    ...
    word-break: break-word;"><?php echo $re['ghichudon'] ;?></span></td>    
		  ...
		  ?>
		   ...
		   ?>
            ...
            <button type="button" <?php echo $disbledkhoa;?> id="btnkhoa<?php echo $re['ID'];?>" onclick="cuahangnhandon(<?php echo $re['ID'].",4" ;?>,'','','<?php echo $dudon;?>','<?php echo $chuoiidsp;?>','<?php echo $re['sochungtu'];?>','<?php echo $textghichudon.' '.$re['sochungtu'];?>')">Khóa phiếu</button>
			 <?php
			 }
			 ?>
			 </td> 
