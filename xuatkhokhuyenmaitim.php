...
if ($tam['apdungcuahang'] > 0 && $tam['apdungcuahang'] != $ch) {
	...
	$tam2 = getdong($sql);
	echo "**#-3**#$tam2[macuahang] hạn sử dụng tới ngày $tam[ngayhethan] **#";
	$data->closedata();
	return;
} else if ($tam['sophieu'] != '') {
	echo "**#-2**#$tam[ngay]**#";
} else  if ($tam['ID'] > 0) {   //$iddung==$tam['iddung']

	$dieukienapdungsosanpham = 0;
	if ($tam['sotiendk'] == 1) $dieukienapdungsosanpham = 1;
	if (1 == 1) echo  "**#" . $tam['sotien'] . '**#' . $dieukienapdungsosanpham . '**#' . $tam['ghichu'];
	else echo "**#-4**#Mã Voucher này không thuộc khách hàng   ! **#";
} else echo "**#-1**#   ";
$data->closedata();
return;
