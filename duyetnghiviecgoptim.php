...
//		 
	// 	$sql = "  select a.IDSP,b.Name,a.mahang,b.IDGroup, sum(a.SoLuong) as sl,b.price as Gia from `xuatbanhang` a left join products b on a.IDSP = b.ID left join phieunhapxuat c on c.ID = a.IDphieu  $sql_where group by a.IDSP order by sl  $sapxep " ;
		...
		$sql = "SELECT a.soct,a.lydo,a.sotien,a.tinhtrang,a.ID,a.idcuahang,c.ten as nguoitao,a.chucvu,a.manv , b.name as tencuahang ,DATE_FORMAT(a.ngaytao,'%d/%m/%Y') as ngaytao,DATE_FORMAT(a.ngaygiamdoc,'%d/%m/%Y %H:%i') as ngayduyet,a.ghichu,a.lydothumua,a.lydoketoan,a.lydoketoan,c.Ten  FROM  a left join cuahang b on a.idcuahang=b.id  left join userac c on a.IDTV = c.ID    ".$sql_where." order by  a.ngaytao desc ";
