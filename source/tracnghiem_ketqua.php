...
//=============================khoi tao ======================//  	 Xem  	 Them  	 Cap nhap  	 Tim  	Huy  	Khoa  	In
  ...
  //=======================================================================================		
	...
	{			
		...
		$sql="SELECT Name,ID,IDGroup,ma,IDGroup  FROM  tracnghiem_ketqua WHERE IDGroup='$id_root' and ID != 0 order by Rank desc";
...
//=============================================	
	...
	function tree($id_root, $level)
	{
		...
		$sql="SELECT * FROM  tracnghiem_ketqua WHERE IDGroup='$id_root' and ID != 0 order by Name Asc";
			   ...
			   $ma = "&nbsp;".$result_news["ma"] ."&nbsp;";
				$id = $result_news["ID"] ;
 				...
 				if ( $result_news["IDGroup"] != "0" ){  
  					...
  					<td  align='center' style='display:$q_cn'><a href='default.php?act=tracnghiem_ketqua&id=$id'><img title='Cập nhập' src=\"images/edit.gif\" border='0'></a></td>
  					<td style='display:$q_xoa' align='center'><a href=\"default.php?act=tracnghiem_ketqua&Del=$id\" onclick=\"return ask()\"><img title=\"Xóa Mục\"  src=\"images/delete.gif\" border=\"0\"></a></td>";
				}
				else{
					$name="<td  align='center'>$stt</td>
					<td  style='display:none' align='left'>$ma</td>
					...
					<td  style='display:none;$q_cn' align='center'><a href='default.php?act=tracnghiem_ketqua&id=$id'><img title=\"Cập nhập\" src=\"images/edit.gif\" border='0'></a></td>
					<td style='display:none;$q_xoa' align='center'><a href=\"default.php?act=tracnghiem_ketqua&Del=$id\" onclick=\"return ask()\"><img title=\"Xóa Mục\"  src=\"images/delete.gif\" border=\"0\"></a></td>";
				}	
				   if($mau == "white")
...
{ 	
		...
		if  ($_GET["id"] == "-1")
		{
		  	$sql ="insert into  tracnghiem_ketqua (IDloai,note,IDGroup,Name,images,Rank,ma,NameN,id_user,ngaytao) values ('$IDloai','$note','$IDGroup','$Name','$images','$Rank','$dapandung','$NameN','$idk','$ngaytao')"; 
			$update = $data->query($sql);
			$them = true;
			$tam_soluong = getdong("select ID from tracnghiem_ketqua order by ID DESC limit 0,1");
			...
			$sqldapand ="insert into  tracnghiem_dapan (IDloai,note,IDGroup,Name,images,Rank,ma,NameN,id_user,ngaytao) values ('3','$note','".$tam_soluong["ID"]."','$dapanc','','','','','$idk','$ngaytao')";
			$update = $data->query($sqldapand);
		} 
		else
		{
		  	$sql ="UPDATE  tracnghiem_ketqua SET  IDloai='$IDloai', note ='$note',ma ='$dapandung',IDGroup ='$IDGroup', Name ='$Name',images ='$images',Rank='$Rank',NameN ='$NameN',id_user_sua='$idk',ngaysua='$ngaytao'  where  ID='0$ID'" ;
			$update = $data->query($sql);
			$them = true; 
...
{ 
 		$IDD = $del ;
 		$xoa = true ;
				if (kiemtranhomcon("tracnghiem_ketqua",$IDD,"IDGroup")=="0")
				{ 
 					$sql = "delete from  tracnghiem_ketqua where   ID = '0".$IDD."'" ;
					$update = $data->query($sql);
 				}
				 else
				{
	...
	if ($_REQUEST["id"] == ""  )
	{
  	...
  	//  $template->assign("caymenuedit", $Caytme );	 
		$sql = "SELECT * ,DATE_FORMAT(ngaytao,'%d/%m/%Y %H:%i') as ngaytest FROM tracnghiem_ketqua  order by ngaytao DESC"; 
		//echo $sql;
		...
		{  
			...
			$template->assign("Name",getten("userac",$result_news["id_user"],"Ten"));
			$template->assign("diemso","".$result_news["ma"]."/".$result_news["IDloai"] );
			$template->assign("tile","" .$result_news["IDnhom"]."%" );
			$template->assign("ketqua","" .$result_news["NameN"]);
			$template->assign("ngaytao","" .$result_news["ngaytest"]);
	...
	{ 
		...
		{
			$sql ="SELECT * FROM  tracnghiem_ketqua WHERE ID='".laso($_REQUEST["id"])."'";
