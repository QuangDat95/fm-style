   ...
   foreach($mang as $x)
   {
	...
	//   echo  $tam["ID"]." select ID from phieunhapxuat where SoCT ='$sochungtu' limit 1 " ;	
		...
		{   //  $idgoi <>0 cập nhập
		   ...
		   $sql = " update phieunhapxuat   set  idkho ='$idkho' , IDNhaCC ='$idkhach' , IDNhap ='$id' ,LyDo='$lydoxuat'    ,VAT='$vat' ,GhiChu='$ghichu'  ,NguoiGiao='$nguoigiao' ,ten='$khach[Name]',diachi='$khach[address]' ,TiGia ='$tigia', tenlydo='$tenlydo' ,tenN='$khach[NameN]' ,diachiN='$idban',tientra='$tientra'    where ID = '$idgoi'"  ;
 		   $data->query($sql);	
