...
if ($_POST["cancel"] != "")
{
	$ID = "" ;
	$_GET["id"] ="" ;
	header("location: ?act=themdexuat");
} 
...
{ 		$tongtien=1;
     ...
     WHERE TABLE_SCHEMA = '$db[name]' AND TABLE_NAME = 'duyetdexuat';" ;  
	 $kq = getdong($sql) ;		
    	$sp  = laso($kq['sp']);	
		
		  $sochungtu = $ct.$nam.$thang.".DX".$IDcuahang.".". $sp;  
		$hinhanh='';
		if($hinhcu){
				...
				foreach($tamhinh as $key => $value){
					if($value){
						if(!in_array($value,$hinhcutam)){
							if(unlink("$dd/images/duyetmuasam/".$value)){
		...
		if($_FILES["hinhanh"]){
			 foreach ($_FILES["hinhanh"]["error"] as $key => $error) 
			{
				if ($error == UPLOAD_ERR_OK) 
				{
					...
					move_uploaded_file($tmp_name, "$dd/images/duyetmuasam/".$name);				
				}
			}	
		}
		...
		if($_GET["id"] == "-1")
		{
 		  ...
 		  $sql ="insert into duyetdexuat set soct='$sochungtu',lydo='$lydo',manv='$manv',ngaytao ='$ngaytao',IDtao=$IDnhanvien,tinhtrang=111,tinnhan=111,IDNV = '$IDnhanvien',hinhanh='$hinhanh',IDcuahang='$IDcuahang',sotien='$sotien',ghichu='$ghichu',soluong='$soluong'";
...
";
				...
				if($result){
					...
					if($result['status']==200){
						$sql="UPDATE  duyetdexuat  set tinnhan=211 where manv='$manv' and ngaytao ='$ngaytao' and IDtao=$IDnhanvien and lydo='$lydo'"  ;
						$update = $data->query($sql);
					}
				}	
			}
		...
		{	
		  ...
		  $sql = "UPDATE  duyetdexuat  set lydo='$lydo',manv='$manv',ngaytao ='$ngaytao',IDtao=$IDnhanvien,tinhtrang=111,IDNV = '$IDnhanvien',hinhanh='$hinhanh',IDcuahang='$IDcuahang',sotien='$sotien',ghichu='$ghichu',soluong='$soluong' where ID=".$_REQUEST["id"]."  and (IDtao=$IDnhanvien or idtao=7218) and  tinhtrang<444  "  ;	
...
if ($del>0  )
{
	...
	{ 
			...
			$sql = "delete from  duyetdexuat where  ID = '0".$IDD."'  $dieukiem " ;
...

{
	...
	if  ($_REQUEST["id"] == ""  || $them  || $xoa || $_POST["search"] != "" )
	{
		...
		$sql = "SELECT a.id FROM duyetdexuat a ";
 		$sql.=$sql_where;
		//echo $sql ;
	...
	// phan trang===================================================================
		...
		$sql = "SELECT a.tinhtrang,a.ID,a.IDcuahang,a.soluong,a.sotien,a.hinhanh,a.soluong,b.name as tencuahang,DATE_FORMAT(a.ngaytao,'%d/%m/%Y %H:%i') as ngaytao,DATE_FORMAT(a.ngaygiamdoc,'%d/%m/%Y') as ngayduyet,a.lydo,a.lydothumua,a.lydogiamdoc,a.lydoketoan,c.Ten as tenNV, c.Chucvu, c.MaNV,a.IDNV FROM duyetdexuat a left join cuahang b on a.idcuahang=b.id left join userac c on a.IDNV=c.id".$sql_where." ORDER BY a.ID  ";
	//	echo $sql;
...
//=========================================================
		...
		{  
				...
				$template->assign("sotien",number_format($re["sotien"]));
				...
				$template->assign("ngayduyet",$re["ngayduyet"]);
  				$tam=$re['tinhtrang']."111"; $ketoan=$tam[1]; $thumua=$tam[0];$giamdoc=$tam[2]; 
				$tinhtrang="Mới tạo" ;
               if($giamdoc==4) {$tinhtrang="Đã duyệt" ;  }  
				elseif ($ketoan==3)  $tinhtrang="Không duyệt" ;  
				elseif ($ketoan==2)  $tinhtrang="Chờ thông tin" ;  
				elseif ($ketoan==4 )  $tinhtrang="Chờ giám đốc duyệt" ; 
				elseif (($ketoan==1||$ketoan==2) && $thumua==4)  $tinhtrang="Chờ kế toán duyệt" ; 
				elseif ($thumua==3)  $tinhtrang="Không duyệt" ;  
				elseif ($thumua==2)  $tinhtrang="Chờ thông tin" ;  
				elseif ($thumua==4 )  $tinhtrang="Chờ kế toán duyệt" ; 
				elseif ($thumua==1||$thumua==2)  $tinhtrang="Chờ thu mua duyệt" ;				
	...
	{ 
		...
		{
			 $template->assign("chonggoilai", gmdate('d/m/Y H:i:s', time() + 7*3600));
			$sql ="SELECT a.lydo,a.ID,a.sotien,a.soluong,DATE_FORMAT(a.ngaygiamdoc,'%d/%m/%Y') as ngayduyet, a.ghichu,a.hinhanh FROM duyetdexuat a WHERE a.ID='".$_REQUEST["id"]."'";
			...
			$result = $data->fetch_array($query);
			  ...
			  $template->assign("idgoi",$result["ID"]);
				...
				if($value!=""){
					...
					<img style="width:100%" src="images/duyetmuasam/'.$value.'" onclick="setimage(\'images/duyetmuasam/'.$value.'\')" /></div>';
					}
				}
