...
$sql_rows = "SELECT  sum(a.psco)as tongpsco,sum(a.psno)as tongpsno,a.loaitk,b.Name,b.macuahang FROM thuchikt a left join cuahang b on a.loaitk=b.id  
	" . $sql_where . " group by a.loaitk ORDER BY b.macuahang desc";
...
$sql = "SELECT  sum(a.psco)as tongpsco,sum(a.psno)as tongpsno,a.loaitk,b.Name,b.macuahang FROM thuchikt a left join cuahang b on a.loaitk=b.id  
	" . $sql_where . " group by a.loaitk ORDER BY b.macuahang desc limit $start,$limit";
// var_dump($sql);
//echo $sql;
...
$chuoiphantrang .= '  <li><button value="-2" onclick="phantrangAjax(this.value,2)">»</button></li>
  <li><button value="F" onclick="phantrangAjax(' . $totalpage . ',2)">Cuối</button></li>
  	<li><input type="button" onclick="xuatexel(2)" name="search3" style="margin-left: 5px;height: 30px;" id="search3" value="Excel Tổng hợp" /></li>
  </ul> 
	  ...
	  position: sticky;left:0;z-index:1;    height: 480px;overflow:scroll">
			...
			<table border="0" cellpadding="0" cellspacing="0" class="tbchuan table_bc" id="dopcccc" style="width:100%;border-collapse:collapse">
				...
				while ($re = $data->fetch_array($result)) {
					...
					$tienleckthu = $mangTLTRu[$re["loaitk"]]["lethu"] ? $mangTLTRu[$re["loaitk"]]["lethu"] : 0;
					$tienleckchi = $mangTLTRu[$re["loaitk"]]["lechi"] ? $mangTLTRu[$re["loaitk"]]["lechi"] : 0;
					$tienleck = ($tienleckthu - $tienleckchi) + $mangTLCH[$re["loaitk"]];
					$tongtlck += $tienleck;
				...
				?>
					...
					<tr class="tb_tr tb_tr<?= $re["idthuchikt"] ?>" data-id="<?= $re["idthuchikt"] ?>" bgcolor="<?php echo $mau; ?>" style="color:<?= $mauchu ?>">
						...
						<td width="150px"><?php echo number_format($mangTongdk[$re["loaitk"]]);  ?></td>
						<td width="150px"><?php echo number_format($mangTLCH[$re["loaitk"]]);  ?></td>
						<td><?php echo number_format($re["tongpsno"]); ?></td>
						<td><?php echo number_format($re["tongpsco"]); ?></td>
						<td width="150px"><?php echo number_format($mangTongdk[$re["loaitk"]] + ($re["tongpsno"] - $re["tongpsco"])); ?></td>
