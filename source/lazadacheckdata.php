	...
	if($check>=1){
			...
			if(!$r['T2']){
				...
				foreach($tammangsp as $key_un => $value_uni){
					if(trim($value_uni['masp'])==trim($r['T5'])){
                  ...
                  /*  if(checkExists('MaNV',$arrmanv["manv"],"userac")){
                    ...
                    */
                    ...
                    $sptam=array(
                        ...
                        //khách hàng
						$checkkh=checkExists('makh',replacesdt(trim($r['T4'])),"customer");
                    if($checkkh){
                        $mangkh=$checkkh;
                    }
                    else{
                    ...
                    //check team
					$mangteam=array('idkho'=>1105,"lydo"=>56,"idchol"=>'');
                    /*if(checkTeam('la56')){
                        $mangteam =checkTeam('la56');
                    }
                    else{
                        if(checkCuahang('la56')){
                            $mangteam =checkCuahang('la56');
                        }
                        else{
...
i
	 $sql="INSERT INTO vanchuyenonline (IDbill,sobill,madh,Fbpage,mavd,madoitac,donvivc,phitravc,phithukh,nvxuly,nvcskh,giamgia,trigiadon,cacmasp,khohang,diachikho,dongthoigiantrangthaidon,tinnhantaodon,ngaydaydon_dvvc,chuyenkhoan,dienthoai_kh,diachi,tinh,quan,phuong,loai)  
	 VALUES('$IDbill','$sobill','$madh','$Fbpage','$mavd','$madoitac','$donvivc','$phitravc','$phithukh','$nvxuly','$nvcskh','$giamgia','$trigiadon','$cacmasanpham','$khohang','$diachikho','$dongthoigiantrangthaidon','$tinnhantaodon','$ngaydaydon_dvvc','$chuyenkhoan','$dtkh','$diachi','$tinh','$quan','$phuong',2)";
	if($IDbill>0) $update=$data->query($sql);
	 	return $update;
}
function GetsoCT(){
	global $data;
