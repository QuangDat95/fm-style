...
if ($thangtruoc==0) $thangtruoc = 12 . "/" .(gmdate('Y', time() + 7*3600)-1) ;
else $thangtruoc = $thangtruoc . "/" .(gmdate('Y', time() + 7*3600)) ;
...
{ 	
     ...
     if($ngay!="")	
		{ 
		  $ngaya=  explode('/',$ngay);	   	 
		  if (strlen($ngaya[0])== 1) { $ngaya[0] = "0".$ngaya[0] ;  } 
 		  $ngay =  "$ngaya[1]-$ngaya[0]-01"; 	
		 }	
		 
		if  ($_GET["id"] == "-1")
		{
			  $sql ="select ID from  kh_thuong where sotien ='$sotien' and idnhanvien='$idnhanvien' and ngay  ='$ngay' and Name ='$Name' limit 1 ";
			  if (getdong($sql)=='')
				{	  
				  $sql ="insert into kh_thuong SET ngaytao='$ngaytao',Name ='$Name', ngay  ='$ngay' ,sotien ='$sotien',idnhanvien='$idnhanvien' ";
				   // echo  $sql;
		...
		{
		  $sql = "UPDATE  kh_thuong  SET  idnhanvien ='$idnhanvien', Name ='$Name', ngay  ='$ngay',sotien ='$sotien' where ID='0$ID'" ;	
		  $update = $data->query($sql);		
		}  
	//	echo $sql ; 
...
{ 
		$IDD = $_GET["Del"] ;
		$sql = "delete from  kh_thuong  where ID = '0".$IDD."'" ;
		$update = $data->query($sql);
		$xoa = true ;
}	

{
	...
	if ($_REQUEST["search"] != "" || $them  || $xoa || $_REQUEST["id"]== "" )
	{
		...
		$sql = "SELECT a.ID FROM   kh_thuong a left join userac  b on a.idnhanvien =b.ID ";
		...
		if($thangtim!="0")
			$sql_where.=" and a.ngay = '".$namtim."-".$thangtim."-01'";
	...
	// phan trang===================================================================
 	 ...
 	 $sql = "SELECT DATE_FORMAT(a.ngay, '%c/%Y') as thang, a.*,b.ten,b.manv FROM kh_thuong a left join userac  b on a.idnhanvien =b.ID  ".$sql_where." ORDER BY a.ngay desc ,b.ten desc  ";
// echo  $sql ;
//=========================================================
  $result = $data->query($sql);
    $SOST = 0 ;
		...
		{  
			...
			$template->assign("thang", $result_news["thang"]);  
	...
	{ 
		...
		else		
		{
			...
			$sql ="SELECT DATE_FORMAT(ngay, '%c/%Y') as ngays, a.*,b.ten,b.manv FROM kh_thuong a left join userac  b on a.idnhanvien =b.ID   WHERE a.ID='".$_REQUEST["id"]."'";
			...
			$result = $data->fetch_array($query);
				...
				$template->assign("ngay".$result["ngays"] , "selected");  	
