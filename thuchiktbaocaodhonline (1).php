...
$data->access();
  ...
  $tmp = explode('*@!',$data1);
   ...
   $mangtk = taomang ("dinhkhoan","ID","madinhkhoan"); 
	  ...
	  $sql_rows = "SELECT g.SoCT as sohd,g.TiGia as giamgiad,DATE_FORMAT(g.ngaytao,'%d/%m/%y %h:%i:%s') as ngayhdbh,b.Name as tencuahang,a.donvivc as dvvc,a.mavandon as mavd,k.Name tenkh,k.tel as sdtkh,i.*,v.phitravc,v.phithukh,v.chuyenkhoan 
 ...
 FROM thuchikt a left join phieunhapxuat g on a.hdbh=g.SoCT  left join vanchuyenonline v on v.IDbill=g.ID left join xuatbanhang i on g.ID=i.IDPhieu left join cuahang b on a.loaitk=b.id  left join userac c on a.IDtao = c.ID 
  left join dinhkhoanthuchi d on d.ID = a.IDcha left join nhacungcap e on a.NCC=e.ID left join phieuxuat  f on a.phieuxuat=f.ID  left join customer k on k.ID=g.IDNhaCC    
	".$sql_where."  and (a.loaitk=1105 or a.loaitk=1137) and g.SoCT is not null ORDER BY g.ID desc  limit $start,$limit";
//echo "<br>".$sql ;
...
$sqlphieuchuaload="select a.*,c.Name as tench,c.macuahang from phieunhapxuat a left join thuchikt b on a.soct=b.hdbh left join cuahang c on a.IDKho=c.ID where ".$sql_where_chuaload." and  b.ID is null and (a.IDKho=1105 or a.IDKho=1137) and a.dakhoa=1";

if($_SESSION["LoginID"]==7576){
  		 echo "<br>".$sqlphieuchuaload."<br>" ;
   }

$resultchuaload = $data->query($sqlphieuchuaload);
$coutnchuaload = $data->num_rows($resultchuaload);

$coutnchuaload =$coutnchuaload ?$coutnchuaload :"0";

$chuoichuaload='<div id="chuaload"><div>
<div id="btn_close_chuaload" >
<button id="" onclick="xuatexel(6)">xuất exel</button>
<button onclick="togglechuaload(false)">X</button>
</div>
<table  border="0" cellpadding="0" cellspacing="0"  class=" table_bc" id="" style="width:100%;border-collapse:collapse">
    <tr align="center" bgcolor="#F8E4CB" class="fixed-top1 ">
      <th  width="68"  valign="middle"  style="height:auto;padding:0.5em">
	  <strong>STT</strong></th> 
	  
      <th width="146" valign="middle"  ><strong>Ngày Nhập</strong></th>
	    <th width="100" valign="middle"  ><strong>Số Hóa đơn</strong></th>
	  <th width="186" valign="middle" ><strong>Mã cửa hàng</strong></th> 
		  ...
		  <th width="153" valign="middle" ><strong>Đơn giá</strong></th>
		    <th width="153" valign="middle" ><strong>Số lượng</strong></th>
			  ...
			  <th width="153" valign="middle" ><strong>Thanh toán</strong></th>
			   <th width="153" valign="middle" ><strong>phí trả NVC</strong></th>
			    <th width="153" valign="middle" ><strong>Phí thu KH</strong></th>
				 <th width="153" valign="middle" ><strong>Chuyển khoản</strong></th>
  ...
  while($re = $data->fetch_array($result))
	...
	$lydoN='';
		...
		if($soctam==""){
		 ...
		 if($soctam!=$re["sohd"]){
					$chuoidong1.='<td>'.number_format($tongtiendon).'</td>
							<td>'.number_format($giamgiatam).'</td>
						<td>'.number_format($tongtiendon-$giamgiatam).'</td>
						<td>'.number_format($phitranvctam).'</td>
						<td>'.number_format($phithukhtam).'</td>
						<td>'.number_format($chuyenkhoantam).'</td>
					</tr>';
				$chuoidong1.=$chuoidong2;
				$chuoidongchinh.=$chuoidong1;
				$chuoidong1='';
				$giamgiatam=0;
				$chuoidong1='<tr  class="tb_tr tb_tr'.$re["idthuchikt"].'" data-id="'.$re["idthuchikt"].'"  bgcolor="'.$mau.'"  style="color:'.$mauchu.'">
			<td>'.$r.'</td>
				...
				if($t==($numrow-1)){
						...
						/*$chuoidong2.='<tr  class="tb_tr tb_tr'.$re["idthuchikt"].'" data-id="'.$re["idthuchikt"].'"  bgcolor="'.$mau.'"  style="color:'.$mauchu.'">
					<td>'.$r.'</td>
						<td>'.$re["ngayhdbh"].'</td>
						<td>'.$re["sohd"].'</td>
						<td>'.$re["tencuahang"].'</td>
