...
if(isset($_POST['DATA'])){
	...
	if($idkho!='1105' && $_SESSION["LoginID"]!='7576' && $_SESSION["LoginID"]!='4647'){
...
<strong style="color:#F90">Đọc dữ liệu từ dòng <?php echo $rows_begin; ?> đến dòng <?php echo $rows_end; ?>  của file Excel</strong> <br />
...
<table id="tbex" width="100%" border="0" cellpadding="0" cellspacing="0" style="background:#FFF;" class="tbchuan">		
			   ...
			   <td width="72" align="center" ><strong>MÃ NV</strong></td>
			    <td width="72" align="center" ><strong>Phiếu xuất</strong></td>
				  <td width="72" align="center" ><strong>Phí thu KH</strong></td>
				    <td width="72" align="center" ><strong>Ghi chú chi tiết</strong></td>
					  <td width="72" align="center" ><strong>người giao</strong></td>
					    <td width="72" align="center" ><strong>Người nhận</strong></td>
						  <td width="72" align="center" ><strong>ngày bắt đầu TS</strong></td>
						    <td width="72" align="center" ><strong>ngày kết thúc TS</strong></td>
							  <td width="72" align="center" ><strong>tên ts</strong></td>
							    <td width="72" align="center" ><strong>nhóm ts</strong></td>
	...
	foreach($sheets as $k => $r) {
		...
		if(!trim($r[0]) || !trim($r[1]) || !trim($r[2]) || !trim($r[3]) || !trim($r[4]) || !trim($r[5])){
					$mauchu='red';
						$baoloirong .='Thiếu dữ liệu!';
						$checkloi=false;
					$loi=true;
		}
			if(trim($r[3])=='DTBH'){
			...
			if(trim($r[3]) && trim($r[3])!=''){
				...
				if($r[9]){
					if(!$mangdv[strtolower(trim($r[9]))]){
						$mauchu='red';
						$baoloirong .='Đơn vị không tồn tại!';
						$checkloi=false;
						$loi=true;
					}
				}
				
					$kiemtracuahang=$mangch[trim($r[1])];
					...
					if(!$kiemtracuahang){
						...
						if($_SESSION["LoginID"]!=7576 && $_SESSION["LoginID"]!=1 && $_SESSION["LoginID"]!=4647 && !$ql[5]){
								if($kiemtracuahang!=$_SESSION["se_kho"]){
									$mauchu='red';
...
//					  echo "từ: ".strtotime($loadtu)."<br/>";
			...
			echo create_slug(trim($r[21]))."<br>";*/
									...
									$checktiendung=true;
												...
												echo "</pre>";*/
													if($tienkiemtrapsno){
														...
														else if($tienkiemtrapsno<=(abs($tiendonhang['thanhtien'])+1000) && $tienkiemtrapsno>=(abs($tiendonhang['thanhtien'])-1000)){
														...
														}else if($tienkiemtrapsco<=(abs($tiendonhang['thanhtien'])+1000) && $tienkiemtrapsco>=(abs($tiendonhang['thanhtien'])-1000) ){
													...
													if(!$checktiensai){
														$mauchu='red';
															array_push($chuoisaiso,trim($r[13]));
					...
					if(!$checkloi){
						...
						$onclick="xuatbaoloi('sai số lẻ! vui lòng xác nhận tải lên sai số!')";
					}
					//$onclick="xuatbaoloi('".$baoloirong."')";
					
		?>
			<tr id="<?php echo $saiso; ?>" bgcolor="<?php echo $maubg; ?>" style="cursor:pointer;color:<?=$mauchu?>" onclick="<?=$onclick?>">
...
$countsaiso=count($chuoisaiso);

?>
<div style="padding-bottom:10px;padding-left:20px">


<input type="button" style="    background-color: #ff9800;
    color: #ffffff;
    border: 1px solid #ff9800;" id="" name="dulieue"  onclick="timdongsaiso('<?php echo implode("###",$chuoisaiso); ?>')" value="Phát hiện (<?php echo $countsaiso;?>) phiếu sai số! click để tìm dòng"/> 

<button type="button" id="dulieue" name="dulieue"  onclick="xuatbaoloi('Vui lòng xác nhận tải lên sai số!')"> Lấy dữ liệu Excel</button> 


<input type="button" style="    background-color:green;
    color: #ffffff;
    border: 1px solid #ff9800;" id="" name="dulieue"  onclick="xacnhantailensaiso('dulieue')" value="Xác nhận tải lên sai số"/> 
</div>  
...
if($checkbangluuco){
  ...
  as thanhtien from phieubanhangluu a where a.SoCT='$soct' and a.dakhoa=1 group by a.SoCT ";
	...
	$sql="select sum(DonGia) as tongtiendg,(sum(ceil(a.DonGia*(1-1*a.chietkhau/100))*a.SoLuong)-b.tigia)
	  as thanhtien from xuatbanhang a left join phieunhapxuat b on a.IDphieu=b.ID and b.dakhoa=1 where b.SoCT='$soct' group by a.IDphieu ";
