 ...
 //==================mang tang ca ============================================				
  ...
  //==============================================================		
		...
		{      
			...
			$sql = "SELECT b.ten as name,a.ngaytao,a.loai,a.thongtin,a.IDnhanvien,a.manv,a.IDcuahang ,DATE_FORMAT( a.ngaytao,'%d/%m/%Y') as ngay,DATE_FORMAT( a.ngaytao,'%e') as ngaylam,DATE_FORMAT( a.ngaytao,'%H') as gio,DATE_FORMAT( a.ngaytao,'%i') as phut,b.chucvu   FROM nhanviendilam a left join userac b on b.ID = a.IDnhanvien ".$sql_where." ORDER BY  a.IDnhanvien,a.ngaytao asc    ";
	...
	//gio quet
	 $gioquetngay= $re['gio']*60+$re['phut'];
		...
		if($ngaythu!=$ngaythuluu)
		{
			...
			if(count($mangtam)>0){
				$mangtam['quettruocsang']=$quettruocsang;
				$mangtam['quetsang']=$quetsang;
				$mangtam['quettrua']=$quettrua;
				$mangtam['quetchieu']=$quetchieu;
				$mangtam['quetsauchieu']=$quetsauchieu;
			
			}
			
			//xủ lý dư liệu thòi gian tại đây
			//in($mangtam);
			if($mangtam['quettruocsang']>0){
				$giovao =strtotime($mangtam['truocsang'][($mangtam['quettruocsang']-1)]['ngaytao']);
				$mangnv[$idnv][$mangtam['ngaylam']]['giovao'.$lanthu]=$mangtam['truocsang'][($mangtam['quettruocsang']-1)]['ngaytao'];
				
			}
			if($mangtam['quetsang']>0){
				$lanthu++;
					if($giovao){
							$giora = strtotime($mangtam['sang'][0]['ngaytao']);
							$sophut=$giora-$giovao;
							$sp= $sogiolam*60-5400; 
							 $giora= $value['gio']*60+$value['phut'];
							 $mangnv[$idnv][$mangtam['ngaylam']]['giora'.$lanthu]=$mangtam['sang'][0]['ngaytao'];
							$mangnv[$idnv][$mangtam['ngaylam']]["thieugio"]='Thiếu giờ';
							 $mangnv[$idnv][$mangtam['ngaylam']]['giolamd'] += $sophut;
							 $mangnv[$idnv]['tongcong']+= $sophut;
							 $giovao='';
					}
					else{
						$giovao =strtotime($mangtam['sang'][0]['ngaytao']);
						$mangnv[$idnv][$mangtam['ngaylam']]['giovao'.$lanthu]=$mangtam['sang'][0]['ngaytao'];
						$mangnv[$idnv][$mangtam['ngaylam']]["ditre"]=$mangtam['sang'][0]['gio'].".".$mangtam['sang'][0]['phut'];
					}
			}
				
			if($mangtam['quettrua']>1){
				
				$lanthu++;
				if($giovao){
						$giora =strtotime($mangtam['trua'][0]['ngaytao']);
						$sophut=$giora-$giovao;
						$sp= $sogiolam*60-5400; 
						$giora= $value['gio']*60+$value['phut'];
						 $mangnv[$idnv][$mangtam['ngaylam']]['giora'.$lanthu]=$mangtam['trua'][0]['ngaytao'];
						 $mangnv[$idnv][$mangtam['ngaylam']]['giolamd'] += $sophut;
						 $mangnv[$idnv]['tongcong']+= $sophut;
						 $giora='';
						 $giovao =strtotime($mangtam['trua'][($mangtam['quettrua']-1)]['ngaytao']);
						$mangnv[$idnv][$mangtam['ngaylam']]['giovao'.$lanthu]=$mangtam['trua'][($mangtam['quettrua']-1)]['ngaytao'];
				}
				else{
					 $giovao =strtotime($mangtam['trua'][($mangtam['quettrua']-1)]['ngaytao']);
					 $mangnv[$idnv][$mangtam['ngaylam']]['giovao'.$lanthu]=$mangtam['trua'][($mangtam['quettrua']-1)]['ngaytao'];
				...
				if($giovao){
						...
						$giovao =strtotime($mangtam['trua'][0]['ngaytao']);
						$mangnv[$idnv][$mangtam['ngaylam']]['giovao'.$lanthu]=$mangtam['trua'][0]['ngaytao'];
				...
				if($giovao){
						...
						$sp= $sogiolam*60-5400; 
						 $giora= $value['gio']*60+$value['phut'];
						 $mangnv[$idnv][$mangtam['ngaylam']]['giora'.$lanthu]=0;
						 $mangnv[$idnv][$mangtam['ngaylam']]['giolamd'] += $sophut;
						 $mangnv[$idnv]['tongcong']+= $sophut;
						 $giovao ="";
				}
			...
			if($mangtam['quetchieu']>0){
				$lanthu++;
					if($giovao){
						
							$giora = strtotime($mangtam['chieu'][0]['ngaytao']);
							$sophut=$giora-$giovao;
							$sp= $sogiolam*60-5400; 
							 $giora= $value['gio']*60+$value['phut'];
							 $mangnv[$idnv][$mangtam['ngaylam']]['giora'.$lanthu]=$mangtam['chieu'][0]['ngaytao'];
							$mangnv[$idnv][$mangtam['ngaylam']]["thieugio"]='Thiếu giờ';
							 $mangnv[$idnv][$mangtam['ngaylam']]['giolamd'] += $sophut;
							 $mangnv[$idnv]['tongcong']+= $sophut;
							 $giovao='';	
						...
						$giovao =strtotime($mangtam['chieu'][0]['ngaytao']);
						$mangnv[$idnv][$mangtam['ngaylam']]['giovao'.$lanthu]=$mangtam['chieu'][0]['ngaytao'];
						$mangnv[$idnv][$mangtam['ngaylam']]["ditre"]=$mangtam['chieu'][0]['gio'].".".$mangtam['chieu'][0]['phut'];
			...
			if($mangtam['quetsauchieu']>0){
				$lanthu++;
					if($giovao){
							$giora = strtotime($mangtam['sauchieu'][0]['ngaytao']);
							$sophut=$giora-$giovao;
							$sp= $sogiolam*60-5400; 
							 $giora= $value['gio']*60+$value['phut'];
						 	$mangnv[$idnv][$mangtam['ngaylam']]['giora'.$lanthu]=$mangtam['sauchieu'][0]['ngaytao'];	
							  $mangnv[$idnv][$mangtam['ngaylam']]['giolamd'] += $sophut;
							 $mangnv[$idnv]['tongcong']+= $sophut;
							 $giovao='';
							 $ravao=true;
			...
			/*foreach($mangtam['sang'] as $key => $value){
				
			}
			foreach($mangtam['trua'] as $key => $value){
				
			}
			foreach($mangtam['chieu'] as $key => $value){
				
			}*/
			//trả dư liệu về lại ban đâu
			 $mangtam=[]; 
			 $quetsang=0;
			$quettrua=0;
			$quetchieu=0; 
		...
		{    
		 ...
		 if ($re['ngay'] != $ngay)  {  $tongngay ++; $ngay = $re['ngay']; }
			...
			//thong tin chung
			$ngaythu =$re['ngaylam'];	
			$ktbuoi=kiemtrasolanquet($giobatdau1,$giobatdau2,$gioketthuc1,$gioketthuc2,$gioquetngay,$quettruocsang,$quetsang,$quettrua,$quetchieu,$quetsauchieu);
				$tamarr=[];
				$tamarr['gio']=$re['gio'];
				$tamarr['phut']=$re['phut'];
				$tamarr['ngaytao']=$re['ngaytao'];
				$mangtam['ngaylam']=$re['ngaylam'];
				$tamarr['gioquetngay']=$gioquetngay;
			
			if($ktbuoi==1){
				if(!$mangtam['truocsang']){
					$mangtam['truocsang']=array();
				}
				array_push($mangtam['truocsang'],$tamarr);
			}	
			else if($ktbuoi==2){
				if(!$mangtam['sang']){
					$mangtam['sang']=array();
				}
				array_push($mangtam['sang'],$tamarr);
			}	
			else if($ktbuoi==3){
				if(!$mangtam['trua']){
					$mangtam['trua']=array();
				}
				array_push($mangtam['trua'],$tamarr);
			}
			else if($ktbuoi==4){
				if(!$mangtam['chieu']){
					$mangtam['chieu']=array();
				}
				array_push($mangtam['chieu'],$tamarr);
			}
			else if($ktbuoi==5){
				if(!$mangtam['sauchieu']){
					$mangtam['sauchieu']=array();
				}
				array_push($mangtam['sauchieu'],$tamarr);
			}
				
				/*$tamarr['quetsang']=$quetsang;
				$tamarr['quettrua']=$quettrua;
				$tamarr['quetchieu']=$quetchieu;*/
				//array_push($mangtam,$tamarr);
				/*if($loaigiotam==1){
				
					$kiemtraquet=kiemtraquetthe4lan($giobatdau1,$giobatdau2,$gioketthuc1,$gioketthuc2,$gioquetngay,$lanthu,$quetgiuagio);
				...
				if($loaigiotam==2){
					...
					$kiemtraquet=kiemtraquetthe2lan($giobatdau1,$giobatdau2,$gioketthuc1,$gioketthuc2,$gioquetngay,$lanthu,$quetgiuagio);
					
					if($kiemtraquet['som'])$mangnv[$idnv]["$ngaythu"]["thieugio"]='Thiếu giờ';
					if($kiemtraquet['tre']){$mangnv[$idnv]["$ngaythu"]["ditre"]=$re['gio'].".".$re['phut'];}
				}
				if($loaigiotam==0){
					$kiemtraquet=kiemtraquetthenlan($gioquetngay,$lanthu);
				}
