...
function addfolder($json){
   $linkimg='https://kimhoangvu.net/webhook/facebook/';
   $folder = "imagesfb/".$json["namefb"];
   // echo $folder; return;
...
function upfile($type){
   $linkimg='https://kimhoangvu.net/webhook/facebook/';
   $uploadDirectoryimages='imagesfb/';
   $data=[];
   $selectFolder = "";
...
function deletefile($json){
  ...
  $count=0;
   foreach ($json as $key => $value) {
      $li=str_replace("https://kimhoangvu.net/webhook/facebook/","",$value);
      unlink($li);
       $count++;
   }
   if($count==count($json)){
