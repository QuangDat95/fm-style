...
<fieldset  class="nencon">
	...
	<label style="Color:#FF3300;Font-Weight:Bold;Font-size:15pt;" >Thu Chi Báo Cáo</label></a>
   </legend><div    > 
 

 <form name="frmthuchi" method="post">

   <div style="position:absolute; left: 992px; top: 68px; width:200px" >[<a href="default.php?act=md">Đóng Lại</a>]&nbsp;</div>
...
<img src="images/img.gif" id="Lichtungaytao2" style="cursor: pointer; border: 0px solid red;" title="Date selector"  onclick="displayCalendar(frmthuchi.tungay,'dd/mm/yyyy',this)" />&nbsp;đến
...
<img src="images/img.gif" id="Lichtungaytao2" style="cursor: pointer; border: 0px solid red;" title="Date selector"  onclick="displayCalendar(frmthuchi.denngay,'dd/mm/yyyy',this)" /></span>
<select onkeypress="return chuyentieps(event,'nhomtk')" name="luachon2"  id="luachon2"  style="width:110px" >
   <option value="5" >Tổng hợp </option>
  <option value="1" >Các khoản thu</option>
  <option value="2" >Các khoản chi</option>
</select>
<select onkeypress="return chuyentieps(event,'loaitk2')" name="nhomtk"  id="nhomtk"  style="width:130px" >
  <option value="" >Tất cả nhóm</option>
    <option value="-2" >Thu chi công nợ</option>
 ...
 <select  name="cuahang"  id="cuahang"  style="width:120px" >
  <option value="" >Tất cả</option>
   ...
   Lý do
   <input type="text" title="Click đôi để xoá trắng" name="lydo2" id="lydo2"  class="text" size="15" value=""  onkeypress="return chuyentiep(event,'loaitk')" ondblclick="xoatrang(this)"/>
<input type="button"   style="width:50px"  onclick="timphieu(tungay.value,denngay.value,luachon2.value,nhomtk.value,cuahang.value,lydo2.value,0)"  name="search2"  id="search2" value="Tìm" />
	    <input type="button" style="font-size: 12px; width: 50px;" id="xuat" value="Excel" name="xuat" onclick="xuatkq()" />
	    <input type="button" style="font-size: 12px; width: 40px;" id="inan" value="IN" name="inan" onclick="goiin()" />
	    </div>
...
<form name="xuatketqua" id="xuatketqua" action="xuatfile.php" method="post" target="_blank" >
  ...
  <input name= "tenfile" id="tenfile" type="hidden" value="thuchibaocao.xls">
...
{   
    poststr="SX="+   t0 ;
    loadtrang('hienthitim', "thuchibaocaotim", poststr,"") ;
}
function xuatexcel()
{
	var st ;
	st = "thuchiexcel.php?h0=" + h0 + '&h1=' + h1+ '&h2=' + h2+ '&h3=' + h3+ '&h4=' + h4+ '&h5=' + encodeURIComponent(h5) ;
...
{
	var st ;
	st = "thuchiexcelquy.php?h0=" + h0 + '&h1=' + h1+ '&h2=' + h2+ '&h3=' + h3+ '&h4=' + h4+ '&h5=' + encodeURIComponent(h5) ;
 	window.open(st,'popup','toolbar=no,status=no,scrollbars=yes,location=no,menubar=no,directories=no,width=1000,height=600,titlebar=no') ;
...
{ 		
	var st ;
	st = "thuchiin.php?t0=" + h0 + '&h1=' + h1+ '&h2=' + h2+ '&h3=' + h3+ '&h4=' + h4+ '&h5=' + encodeURIComponent(h5) ;
	window.open(st,'popup','toolbar=no,status=no,scrollbars=yes,location=no,menubar=no,directories=no,width=1000,height=600,titlebar=no') ;
}function xuly1()
{
...
{  //tungay,denngay,luachon,nhomtk,taikhoan2,lydo2,tr
 ...
 if(thongbao(cf) == false) { return }	 
    ...
    loadtrang('ketqualuu',"thuchiluu",poststr,"xuly1") ;		
}

function timphieu(t0,t1,t2,t3,t4,t5,t6)
{  //tungay,denngay,luachon,nhomtk,cuahang,lydo2,tr
	...
	poststr= poststr + "*@!"+encodeURIComponent(t4)+  "*@!" + encodeURIComponent(t5)+  "*@!"+ encodeURIComponent(t6)+  "*@!"+ encodeURIComponent(0);
    loadtrang('hienthitim',"thuchibaocaotim", poststr,"") ;		
}
