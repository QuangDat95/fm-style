...
//$ghtk=new Ghtk("187c69cA1c3d49fE1B43573b335d67a7481e7181","");
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   $now=date('Y-m-d H:i:s') ;
	  ...
	  else if($loai==4) 
	 {
		 ...
		 //khóa phiếu
		 $phivcthukhach=getten("passdon",$id,"phivcthukhach");
		/*if(!$tienship){
			 if(!$phivcthukhach || $phivcthukhach==''){
				echo  "###4###vui lòng nhập phí ship!###$id###$tench###$idcuahang###-1###" ;
				return;
			}
			else{
				$tienship=$phivcthukhach;
			}
		}*/
		$tienship=$phivcthukhach;
	...
	//	 echo "Trùng số chứng từ !!! " ; return ; }   
		//=======================================================================================
		  			 $thang = gmdate('m', time() + 7*3600); 
					 $thang=strlen($thang)==1?"0".$thang:$thang;
				 ...
				 //   echo $kq['sp'];	
				   ...
				   $sochungtu2 ="B".$nam.$thang.$_SESSION["S_MaNV"].".".$idkho.".".$sp1.($sp+1) ; 
							...
							echo "sct1: ". $sochungtu."<br/>";
							echo  "sct2: ".$sochungtu2."<br/>";
						   $tam = getdong(" select ID from phieunhapxuat where SoCT ='$sochungtu' limit 1 ") ;
...
//				);
					if(!$update){
				 			...
				 			echo "###4###Số chứng từ $SoCTNew đã sư dụng vui lòng thực hiện thao tác khóa phiếu lại!###$id###$tench###$idcuahang###-1###$idpasschitiet###";
								return;
					}else if($update){
				...
				$datagetfee["address"]=$diachikh["address"];
				if(in_array($diachikh["matinh"],$mangtpTrunguong)){
					$mangoder['province']="TP. ".$diachikh["tinh"];
					$datagetfee["province"]="TP. ".$diachikh["tinh"];
				}
				else{
					$mangoder['province']="Tỉnh ".$diachikh["tinh"];
					$datagetfee["province"]="Tỉnh ".$diachikh["tinh"];
				}
				
				$mangoder['district']=$diachikh["quan"];
				$mangoder['ward']=$diachikh["phuong"];
				$datagetfee["district"]=$diachikh["quan"];
				//$datagetfee["ward"]=$diachikh["phuong"];
				//+++++++++++++++++++++
				...
				$datagetfee["pick_province"]=$diachich["tinh"];
				if(in_array($diachich["matinh"],$mangtpTrunguong)){
					$datagetfee["pick_province"]="TP. ".$diachich["tinh"];
					$mangoder['pick_province']="TP. ".$diachich["tinh"];
				}
				else{
					$mangoder['pick_province']="Tỉnh ".$diachich["tinh"];
					$datagetfee["pick_province"]="Tỉnh ".$diachich["tinh"];
				}
				$mangoder['pick_district']=$diachich["quan"];
				$mangoder['pick_ward']=$diachich["phuong"];
				
				$datagetfee["pick_district"]=$diachich["quan"];
				$datagetfee["pick_ward"]=$diachich["phuong"];
				$datagetfee["weight"]=0.5;
				$datagetfee["transport"]="road";
			...
			$mavd="";
			//$mavdres=$ghtk->CreatBillCode($orderArr);
			/*if($mavdres["success"]){
				$mavd=$mavdres["order"]["label"];
			}*/
				...
				echo "</pre>";*/
			$phivcdvvc=0;
			/*$arrphivcdvvc=getfeevcv(json_encode($datagetfee));
			if($arrphivcdvvc["success"]){
				$phivcdvvc=$arrphivcdvvc["fee"]["fee"];
			}*/
			
			$sql="update passdon set phivcdvvc='$phivcdvvc',phivcthukhach='$tienship',dakhoa=1 where id=$id";
			$update= $data->query($sql);
			
			$sql="insert into vanchuyenpassdon(IDbill,IDpassdon,mavd,ngaytao,donvivc,phiship,IDTao,phivcdvvc) values ('$idphieu','$id','$mavd','$NgayTao','GHTK','$tienship','$idk','$phivcdvvc')";
			...
			$update= $data->query($sql);
				...
				if($update){
					...
					$khohang=getten("cuahang",$idcuahang,'Name');
						...
						if($update){
							/*if(!$mavdres["success"]){
							echo  "###4###Đã Khóa phiếu! ".$mavdres["message"]."###$id###$tench###$idcuahang###1###$idpasschitiet###" ;
							}
							else{*/
			 ...
			 foreach($mangtam as $key => $value){
				...
				select '$key',IDNhaCC,IDNhap,NgayNhap,'$SoCTNew',LyDo,SoNgayNo,IDTKNo,IDTKCo,TiGia,VAT,'$lydo','$NgayTao',IDTao,NguoiGiao,Loai,ten,diachi,tenkho,tenlydo,tenN,diachiN,1,nguoitao,nguoisua,ngaykhoa,dahuy,IDkhoa,tientra,idchOL,idgioithieu from passdon where ID='$id'";
					...
					if(!$update){
							...
							//$diachich["address"]=str_replace("hàng","hàngg",$diachich["address"]);
							//$diachich["address"]=str_replace("Hàng","Hàngg",$diachich["address"]);
							$mangoder['pick_address']=$address;
							$mangoder['pick_province']=$diachich["tinh"];
							$datagetfee["pick_province"]=$diachich["tinh"];
...
curl_setopt_array($curl, array(
    CURLOPT_URL => "https://services.giaohangtietkiem.vn/services/shipment/fee",
