...
//=============================khoi tao ======================//  	 Xem  	 Them  	 Cap nhap  	 Tim  	Huy  	Khoa  	In
  ...
  //=======================================================================================		
	...
	{			
		...
		$sql="SELECT Name,ID,IDGroup,ma,IDGroup  FROM  nhomcongvan WHERE IDGroup='$id_root' and ID != 0 order by Rank desc";
...
//=============================================	
	...
	function tree($id_root, $level)
	{
		...
		$sql="SELECT * FROM  nhomcongvan WHERE IDGroup='$id_root' and ID != 0 order by Rank desc";
 			...
 			{   
			   $ma = "&nbsp;".$result_news["ma"] ."&nbsp;";
				...
				{  
					...
					<td  align='left'>$ma</td><td>&nbsp; $name1
					<img src='images/images_admin/nutdo.gif' border='0'>&nbsp;$mottin</td>
					<td  align='center' style='display:$q_cn'>
					<a href='default.php?act=nhomcongvan&id=$id'>
					<img title='Cập nhập' src=\"images/edit.gif\" border='0'></a></td>
					<td style='display:$q_xoa' align='center'><a href=\"default.php?act=nhomcongvan&Del=$id\" onclick=\"return ask()\">
					  <img title=\"Xóa Mục\"  src=\"images/delete.gif\" border=\"0\"></a></td>";
				}else
				{				
					$name="<td  align='center'>$stt</td>
					<td align='left'>$ma</td><td>&nbsp; $name1
					<a href='default.php?act=nhomcongvan&id=$id'>
					<img src=\"images/images_admin/round_f.gif\" border='0'>
					</a>
					
					<strong>&nbsp;$mottin</strong></td><td  style='display:$q_cn' align='center'><a href='default.php?act=nhomcongvan&id=$id'><img title=\"Cập nhập\" src=\"images/edit.gif\" border='0'></a></td><td style='display:$q_xoa' align='center'><a href=\"default.php?act=nhomcongvan&Del=$id\" onclick=\"return ask()\"><img title=\"Xóa Mục\"  src=\"images/delete.gif\" border=\"0\"></a></td>";
				}	
				   if($mau == "white")
...
{ 	
		...
		if  ($_GET["id"] == "-1")
		{
		  $sql ="insert into  nhomcongvan (IDnhom,note,IDGroup,Name,images,Rank,ma,NameN) values ('$IDnhom','$note','$IDGroup','$Name','$images','$Rank','$ma','$NameN')";
		} else
		{
		  $sql ="UPDATE  nhomcongvan SET  IDnhom='$IDnhom', note ='$note',ma ='$ma',IDGroup ='$IDGroup', Name ='$Name',images ='$images',Rank='$Rank',NameN ='$NameN' where  ID != 1 and   ID='0$ID'" ;			
		}  
...
{ 
 		$IDD = $del ;
 		$xoa = true ;
				if (kiemtranhomcon("nhomcongvan",$IDD,"IDGroup")=="0")
				{ 
 					$sql = "delete from  nhomcongvan where ID != 1   and  ID = '0".$IDD."'" ;
					$update = $data->query($sql);
 				}
				 else
				{
	...
	{ 
		...
		{
			$sql ="SELECT * FROM  nhomcongvan WHERE ID='".laso($_REQUEST["id"])."'";
