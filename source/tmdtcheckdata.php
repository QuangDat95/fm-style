...
$sp=laso(GetsoCT())  ;  if ($sp==0) $sp =1;
echo  $sp ."=======";
$ngaytao = gmdate('Y-m-d H:i:s', time() + 7*3600) ;
	$ngaynhap = gmdate('Y-m-d', time() + 7*3600) ;
	...
	if($check>=1){
			...
			if(!$r['T2']){
				...
				foreach($tammangsp as $key_un => $value_uni){
					if($value_uni['masp']==$r['T5']){
							
						$tammangsp[$key_un]['soluong']=$value_uni['soluong']+$r['T7'];
				...
				if($check_uniq_sp){
					$sql="select Name,ID,price,giabinhquan,codepro,IDGroup,code from products where codepro ='$r[T5]' limit 1";
				$dong=getdong($sql);
					$sptam=array(
						"IDSP"=> $dong['ID'] ,
						"masp"=> $dong['codepro'] ,
                  ...
                  /*  if(checkExists('MaNV',$arrmanv["manv"],"userac")){
                    ...
                    */
                    ...
                    $sptam=array(
                        ...
                        //khách hàng
                    if(checkExists('makh',$r['T4'],"customer")){
                        $mangkh=checkExists('makh',$r['T4'],"customer");
                    }
                    else{
                    ...
                    if(checkTeam('Shoppe')){
                        $mangteam =checkTeam('Shoppe');
                    }
                    else{
                        if(checkCuahang('Shoppe')){
                            $mangteam =checkCuahang('Shoppe');
                        }
                        else{
                            $mangteam=array('idkho'=>'',"lydo"=>'',"idchol"=>'');
                        }
                    }
                    array_push($tammangsp,$sptam);
                    $tammanggiaoh=array(
...
global $data;
 $ngay = gmdate('Y-m-d H:i:s', time() + 7*3600) ;
	$name=$arr["name"];
	$makh=$arr["makh"];
	$address=$arr["address"];
	$type=$arr["type"];
...
function insertVanchuyenonline($arr){
	...
	$chuyenkhoan=$arr['chuyenkhoan'];
	 ...
	 $sql="INSERT INTO vanchuyenonline (IDbill,sobill,madh,Fbpage,mavd,madoitac,donvivc,phitravc,phithukh,nvxuly,nvcskh,giamgia,trigiadon,cacmasp,khohang,diachikho,dongthoigiantrangthaidon,tinnhantaodon,ngaydaydon_dvvc,chuyenkhoan,dienthoai_kh,diachi,tinh,quan,phuong)  
	 VALUES('$IDbill','$sobill','$madh','$Fbpage','$mavd','$madoitac','$donvivc','$phitravc','$phithukh','$nvxuly','$nvcskh','$giamgia','$trigiadon','$cacmasanpham','$khohang','$diachikho','$dongthoigiantrangthaidon','$tinnhantaodon','$ngaydaydon_dvvc','$chuyenkhoan','$dtkh','$diachi','$tinh','$quan','$phuong')";
