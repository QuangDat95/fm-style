...
if($suatdattruoc['tongsuat']>0){
	$nhapnhay='blink';
	$titlenhapnhay='Có suất ăn đặt trước! Click vào để kiểm tra!';
...
$template->assign('muccha',compo11('khuvuc where IDcha=0','ten',$idcha));
	...
	$sql="select count(ID) as tongsuat from suat_an where ((ngaytao>='$ngay' and ngaytao <='$ngay 23:59:00' and dattruoc=0) or (ngaydangky>='$ngay' and ngaydangky <='$ngay 23:59:00' and dattruoc=1 and xacnhan=22)) and (comtrua=1 or comtoi=1)";
	//echo $sql;
	$query=query($sql);
	...
	while($row=fetch_array($query)){
		...
		$tongsuatkv=0;
		$sql1="select count(ID) as tongsuat,maphongban from suat_an where ((ngaytao>='$ngay' and ngaytao <='$ngay 23:59:00' and dattruoc=0) or (ngaydangky>='$ngay' and ngaydangky <='$ngay 23:59:00' and dattruoc=1 and xacnhan=22)) and (comtrua=1 or comtoi=1) and maphongban in (select idphong from khuvuc where IDcha=$row[ID])  group by maphongban";
			$query1=query($sql1);
...
function checktontaikhuvuc_baocao_com($idkv,$ngay){
	$sql="select ID from khuvuc_baocao_com where idkhuvuc='$idkv' and ngay='$ngay' (and dattruoc=0 or (and dattruoc=1 and xacnhan=22)";
...
function getsuatchiakhuvuctru($idkv,$ngay){
	$sql="select sum(a.sosuat) as tongsuat,a.khuvucden,b.ten from khuvuc_chiasuat a left join khuvuc b on a.khuvucden=b.ID where a.khuvucgoc = $idkv and ngay='$ngay' and ( dattruoc=0 or (dattruoc=1 and xacnhan=22)) group by khuvucden";
		$query=query($sql);
		$mang=[];
	while($row = fetch_array($query))
	{
...
where a.khuvucgoc <> $idkv and ngay='$ngay'  and (dattruoc=0 or (dattruoc=1 and xacnhan=22))
