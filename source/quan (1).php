...
{ 	
	...
	if  ($_GET["id"] == "-1")
	{
	  $sql ="insert into quan (Name,loai,idtinh) values ('$Name ','$loai','$idtinh')";
	}else{

		$tinh = $_POST["tinhsl"];
	
	  $sql = "UPDATE  quan  SET  Name ='$Name', loai  ='$loai' , idtinh  ='$idtinh' where ID='0$ID'" ;			
	}  
	 //echo $sql ;
...
{ 
		$IDD = $_GET["Del"] ;
		$sql = "delete from  quan  where ID = '0".$IDD."'" ;
		$update = $data->query($sql);
		$xoa = true ;
}	

{
	...
	
	{
		...
		//$SOST = 0 ;
	// phan trang===================================================================
	     $sql = "SELECT ID FROM quan $sql_where order by Name ";
		...
		$sql = "SELECT quan.ID as IDquan,quan.Name as quan , tinh.Name as tinh, quan.loai as loai
		from quan
		inner join tinh on quan.idtinh = tinh.ID 
		".$sql_where." ORDER BY quan.ID desc , quan.ID limit $page_start,$page_row ";
		
		// SELECT * FROM quan ".$sql_where." ORDER BY id desc, ID limit $page_start,$page_row ";
...
//=========================================================
	...
	{ 
		...
		{
			$sql ="SELECT * FROM  quan  WHERE ID='".$_REQUEST["id"]."'";
